...
B 类地址最高位为 10，使用 14-bit 表示网络号，16-bit 表示本地主机地址，可以分配 16,384 个 A 类网络。
...
C 类地址最高位为 110，使用 21-bit 表示网络号，8-bit 表示本地主机地址，可以分配 2,097,152 个 A 类网络。
...
举例解析 NAT 的原理，假设以下网络，NAT 网关拥有公网端口的 IP 地址 22.20.20.1，私网 LAN 端口 IP 地址 192.168.1.1 是作为私网的网关 Gateway 保留地址。私网其中一台主机 A 192.168.1.2 向公共网中的主机 202.20.65.7 发送一个 IP 包 (Dst 202.20.65.7 => Src 192.168.1.2)。
...
当 IP 包经过 NAT 网关时，IP 包的 Src IP 会被 NAT Gateway 替换成公共 IP 22.20.20.1 并转发到公共网，此时 IP 包已经不含任何私有网 IP 的信息。公网主机 202.20.65.5 收到 IP 包后进行响应，相应 IP 包 (Dst 202.20.65.5 => Src 22.20.20.1) 将被发送到 NAT Gateway。

这时，NAT Gateway 会将 IP 包的目的 IP 转换成私有网中 A 主机的 IP 并将 IP 包 (Des 192.168.1.2 => Src 202.20.65.4) 转发到私有网由目标主机 A 接受。对于通信双方而言，这种地址的转换过程是完全透明的。
