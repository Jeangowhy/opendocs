...
表示 blue 到 green 过渡混色的中间值，也就是青色带点蓝，然后然后再混合 20% 的 red。注意
顺序从右到左，0% 表示只取其右侧的颜色，100% 表示只取左侧的颜色。一般语法格式 `a(!level!b)*`，
...
配合 looseness 属性控制松紧度，0 表示不施加弯曲。使用 TikZiT 绘图工具可以很直观看到曲线的
...
+-----------------------+                   +-----------------------+           
      ...
      Web Browser                           The Elm Architecture (TEA)
