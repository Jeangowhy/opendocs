...
<ul class="badges">
    <li>
        <svg class="vcs_badge" id="badge-untracked" viewBox="0 0 12 12" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:1.41421;">
            <path d="M6,2.047c2.208,0 4,1.793 4,4c0,2.208 -1.792,4 -4,4c-2.208,0 -4,-1.792 -4,-4c0,-2.207 1.792,-4 4,-4Zm0,1c1.656,0 3,1.345 3,3c0,1.656 -1.344,3 -3,3c-1.656,0 -3,-1.344 -3,-3c0,-1.655 1.344,-3 3,-3Z"></path>
        </svg>
        Untracked
    </li>
    <li>
...
Sublime Text’s [incremental\_diff](incremental_diff.html) functionality ties in with the Git integration. By default, the incremental diff functionality tracks changes to the file since it was last saved, but it is also possible to diff against HEAD.
...
See the [incremental\_diff](incremental_diff.html) documentation for more information and examples, including instructions for viewing inline diffs, navigating between hunks and reverting changes.
...
"show\_git\_status" boolean[🔗](){id=doc_show_git_status}

Enables Git integration.

Default: `true`

"git\_diff\_target" string[🔗](){id=doc_git_diff_target}
...
.incremental\_diff { background-color: hsl(210, 15%, 24%); color: hsl(219, 28%, 88%); padding: 5px; font-family: "Roboto Mono", monospace; line-height: 1.3; border-radius: 2px; } .incremental\_diff .line\_number { width: 40px; text-align: right; padding-right: 15px; opacity: 0.5; } .incremental\_diff .diff\_marker { width: 4px; } .incremental\_diff .deleted-before, .incremental\_diff .deleted-after, .incremental\_diff .added, .incremental\_diff .modified { position: relative; } .incremental\_diff .added:after { content: ' '; display: block; width: 3px; height: 100%; background-color: hsl(114, 31%, 68%); position: absolute; top: 0; left: 0; } .incremental\_diff .modified:after { content: ' '; display: block; width: 3px; height: 100%; background-color: hsl(32, 93%, 66%); position: absolute; top: 0; left: 0; } .incremental\_diff .deleted-before:after { content: ' '; display: inline-block; width: 0; height: 0; border: 2px solid transparent; border-left-color: hsl(357, 79%, 65%); border-bottom-color: hsl(357, 79%, 65%); position: absolute; bottom: 0; left: 0; } .incremental\_diff .deleted-after:after { content: ' '; display: inline-block; width: 0; height: 0; border: 2px solid transparent; border-left-color: hsl(357, 79%, 65%); border-top-color: hsl(357, 79%, 65%); position: absolute; top: 0; left: 0; }
...
"mini\_diff" boolean, string[🔗](){id=doc_mini_diff}
...
"index\_files" boolean[🔗](){id=doc_index_files}

If the indexing engine is enabled

_Default:_ `true`

"index\_workers" integer[🔗](){id=doc_index_workers}
...
"index\_exclude\_gitignore" boolean[🔗](){id=doc_index_exclude_gitignore}
...
"index\_skip\_unknown\_extensions" boolean[🔗](){id=doc_index_skip_unknown_extensions}
...
"index\_exclude\_patterns" array of strings[🔗](){id=doc_index_exclude_patterns}

[File Patterns](file_patterns.html) used to exclude files from indexing.

_Default:_ `["*.log"]`

"show\_definitions" boolean[🔗](){id=doc_show_definitions}

If the _Goto Definition_ popup will appear when hovering the mouse over a word that has been indexed

_Default:_ `true`

"auto\_complete\_use\_index"[🔗](){id=doc_auto_complete_use_index}
...
echo 'export PATH="/Applications/Sublime Text.app/Contents/SharedSupport/bin:$PATH"' \>> ~/.bash\_profile
...
If installing from a tarball, the sublime\_text executable should be symlinked to subl, with a command such as:

sudo ln \-s /opt/sublime\_text/sublime\_text /usr/local/bin/subl
...
import sublime
import sublime\_plugin

class MyCompletions(sublime\_plugin.EventListener):
    def on\_query\_completions(self, view, prefix, locations):
        if not view.match\_selector(locations\[0\], "source.python"):
            return \[\]

        available\_completions \= \[
            "def",
            "class",
            "None",
            "True",
        ...
        out \= \[\]
        for comp in available\_completions:
...
"tab\_completion" boolean[🔗](){id=doc_tab_completion}
...
"auto\_complete\_triggers" array of objects[🔗](){id=doc_auto_complete_triggers}
...
"auto\_complete\_commit\_on\_tab" boolean[🔗](){id=doc_auto_complete_commit_on_tab}
...
"auto\_complete\_with\_fields" boolean[🔗](){id=doc_auto_complete_with_fields}
...
"auto\_complete\_cycle" boolean[🔗](){id=doc_auto_complete_cycle}
...
"auto\_complete\_use\_history" boolean[🔗](){id=doc_auto_complete_use_history}

If previously-selected completions should be automatically selected.

"auto\_complete\_use\_index" boolean[🔗](){id=doc_auto_complete_use_index}

4052

When enabled, the completions popup will show context-aware suggestions based on other files in the project.

"auto\_complete\_preserve\_order" string[🔗](){id=doc_auto_complete_preserve_order}
...
"auto\_complete\_trailing\_symbols" boolean[🔗](){id=doc_auto_complete_trailing_symbols}

4050

Add trailing symbols (e.g., `.`, `()`) if the completion engine thinks they‘re likely enough.

"auto\_complete\_trailing\_spaces" boolean[🔗](){id=doc_auto_complete_trailing_spaces}

4050

Add a space after completions if the completion engine thinks they‘re likely enough.

"auto\_complete\_include\_snippets" boolean[🔗](){id=doc_auto_complete_include_snippets}
...
"auto\_complete\_include\_snippets\_when\_typing" boolean[🔗](){id=doc_auto_complete_include_snippets_when_typing}
...
"ignored\_snippets" array of strings[🔗](){id=doc_ignored_snippets}
...
"vintage\_start\_in\_command\_mode": true
...
{
    "keys": \["j", "j"\],
    "command": "exit\_insert\_mode",
    "context":
    \[
        { "key": "setting.command\_mode", "operand": false },
        { "key": "setting.is\_widget", "operand": false }
    \]
}
...
*   [“build\_systems” Key](#build-systems-key)
...

{
    ...
    \[
        {
            "path": "src",
            "folder\_exclude\_patterns": \["backup"\],
            "follow\_symlinks": true
        },
        {
            "path": "docs",
            "name": "Documentation",
            "file\_exclude\_patterns": \["\*.css"\]
        }
    \],
    "settings":
    {
...
"file\_include\_patterns" array of strings[🔗](){id=doc_file_include_patterns}
...
"file\_exclude\_patterns" array of strings[🔗](){id=doc_file_exclude_patterns}
...
"folder\_include\_patterns" array of strings[🔗](){id=doc_folder_include_patterns}
...
"folder\_exclude\_patterns" array of strings[🔗](){id=doc_folder_exclude_patterns}
...
"binary\_file\_patterns" array of strings[🔗](){id=doc_binary_file_patterns}

3158

[Patterns of files](file_patterns.html) to treat as binary files, and thus ignored in _Goto Anything_ and _Find in Files_.

"index\_include\_patterns" array of strings[🔗](){id=doc_index_include_patterns}
...
"follow\_symlinks" boolean[🔗](){id=doc_follow_symlinks}
...
{
    "keys": \["ctrl+k", "ctrl+c"\],
    "command": "set\_setting",
    "args":
    {
        "setting": "color\_scheme",
...
_A key binding for the **Escape** key_
...
div.col { float: left; padding-right: 40px; } div#key\_names-columns:after { content: ""; display: table; clear: both; }
...
_Currently there is no compiled list of all built-in commands. The names of many commands can be found by looking at the Default ({PLATFORM\_NAME}).sublime-keymap files in the Default/ package._
...
If a `"key"` support _equality_ operators, the `"operator"` must be one of: `"equal"` or `"not_equal"`. For _matching_ it must be one of: `"regex_match"`, `"not_regex_match"`, `"regex_contains"` or `"not_regex_contains"`. The default `"operator"` is `"equal"` and the default `"operand"` is `true`.
...
"spell\_check" boolean[🔗](){id=doc_spell_check}
...
"added\_words" array of strings[🔗](){id=doc_added_words}

An array of properly spelled words to augment the dictionary with.

Example: `["unobscurable"]`

"ignored\_words" array of strings[🔗](){id=doc_ignored_words}
...
"file\_patterns" array of strings[🔗](){id=doc_file_patterns}
...
The command to run when the build system is invoked. The default value of `exec` allows use of the additional options specified in [exec Target Options](#exec-target-options).
...
{
    "cmd": \["/Applications/MyProgram.app/Contents/MacOS/my\_command", "-d", "$file"\]
}
...
{
    "cmd": \["/usr/local/bin/my\_command", "-d", "$file"\]
}
...
"shell\_cmd" string[🔗](){id=doc_shell_cmd}
...
This takes precedence over [`"cmd"`](#cmd ""cmd""). If you want to override the [`"shell_cmd"`](#shell_cmd ""shell_cmd"") in a build variant with a [`"cmd"`](#cmd ""cmd"") then also set the [`"shell_cmd"`](#shell_cmd ""shell_cmd"") to `null`.

May use [variables](#variables).

Example: `"my_command \"$file\" | other_command"`

"working\_dir" string[🔗](){id=doc_working_dir}
...
"file\_regex" string[🔗](){id=doc_file_regex}
...
"line\_regex" string[🔗](){id=doc_line_regex}
...
"word\_wrap" boolean[🔗](){id=doc_word_wrap}
...
The following Python can be saved to a file named Package/User/my\_example\_build.py:
...
class MyExampleBuildCommand(sublime\_plugin.WindowCommand):
    ...
    def is\_enabled(self, lint\=False, integration\=False, kill\=False):
    ...
    def run(self, lint\=False, integration\=False, kill\=False):
        ...
        vars \= self.window.extract\_variables()
        working\_dir \= vars\['file\_path'\]

        \# A lock is used to ensure only one thread is
        \# touching the output panel at a time
        with self.panel\_lock:
            \# Creating the panel implicitly clears any previous contents
            self.panel \= self.window.create\_output\_panel('exec')

            \# Enable result navigation. The result\_file\_regex does
            \# the primary matching, but result\_line\_regex is used
            \# when build output includes some entries that only
            \# contain line/column info beneath a previous line
            \# listing the file info. The result\_base\_dir sets the
            ...
            settings.set('result\_base\_dir', working\_dir)

            self.window.run\_command('show\_panel', {'panel': 'output.exec'})
        ...
        threading.Thread(
            target\=self.read\_handle,
            args\=(self.proc.stdout,)
        ).start()

    def read\_handle(self, handle):
        chunk\_size \= 2 \*\* 13
        out \= b''
        while True:
            try:
                data \= os.read(handle.fileno(), chunk\_size)
                \# If exactly the requested number of bytes was
                \# read, there may be more data, and the current
                \# data may contain part of a multibyte char
                out += data
                if len(data) \== chunk\_size:
                    continue
                if data \== b'' and out \== b'':
                    raise IOError('EOF')
                ...
                self.queue\_write(out.decode(self.encoding))
                if data \== b'':
                    raise IOError('EOF')
                out \= b''
            except (UnicodeDecodeError) as e:
                msg \= 'Error decoding output using %s - %s'
                self.queue\_write(msg  % (self.encoding, str(e)))
                break
            except (IOError):
                if self.killed:
                    msg \= 'Cancelled'
    ...
    def queue\_write(self, text):
        sublime.set\_timeout(lambda: self.do\_write(text), 1)

    def do\_write(self, text):
        with self.panel\_lock:
            self.panel.run\_command('append', {'characters': text})
...
*   _<executable\_path>_/Packages/
*   _<data\_path>_/Installed Packages/
    

Loose packages may be stored in:

*   _<data\_path>_/Packages/
    

For example, the package Python is stored in _<executable\_path>_/Packages/Python.sublime-package, and any files in the _<data\_path>_/Packages/Python/ directory will override those stored in the .sublime-package file.

In general, _<executable\_path>_/Packages/ is for packages that ship with Sublime Text, and _<data\_path>_/Installed Packages/ is for packages installed by the user.
...
To create a new package, simply create a new directory under _<data\_path>_/Installed Packages/. You can access this directory from the Preferences ![▶](/images/right.svg) Browse Packages menu.
...
For example to override the file function.sublime-snippet in the Python.sublime-package package that ships with Sublime Text, create a directory called Python under the _<data\_path>_/Packages/ directory, and place your function.sublime-snippet file there.
...
Sublime Text runs on the Windows, Mac and Linux operating systems. 32bit (x86), 64bit (x86\_64) and ARM (arm64) and 64bit (x86\_64) processors are supported.
...
_Sublime Text 4 is a Universal 2 binary that supports both Intel(x86\_64) and Apple Silicon (arm64) processors._

_Only 64bit (x86\_64) builds are available on Mac._
...
_Both 64bit (x86\_64) and ARM (arm64) builds are now offered on Linux. 32bit builds have been discontinued._
...
*   **Stable x86\_64**
    ...
    echo \-e "\\n\[sublime-text\]\\nServer = https://download.sublimetext.com/arch/stable/x86\_64" | sudo tee \-a /etc/pacman.conf
    
*   **Dev x86\_64**
    
    echo \-e "\\n\[sublime-text\]\\nServer = https://download.sublimetext.com/arch/dev/x86\_64" | sudo tee \-a /etc/pacman.conf
...
*   **Stable**
    ...
    sudo yum-config-manager \--add-repo https://download.sublimetext.com/rpm/stable/x86\_64/sublime-text.repo
    
*   **Dev**
    
    sudo yum-config-manager \--add-repo https://download.sublimetext.com/rpm/dev/x86\_64/sublime-text.repo
...
*   **Dev**
    ...
    sudo dnf config-manager \--add-repo https://download.sublimetext.com/rpm/dev/x86\_64/sublime-text.repo
...
*   **Stable**
    ...
    sudo zypper addrepo \-g \-f https://download.sublimetext.com/rpm/stable/x86\_64/sublime-text.repo
    
*   **Dev**
    
    sudo zypper addrepo \-g \-f https://download.sublimetext.com/rpm/dev/x86\_64/sublime-text.repo
...
var BASE\_URL = 'https://download.sublimetext.com' var LINK\_LIST\_ELEMENT = document.getElementById("link-list"); var VERSION\_INPUT\_ELEMENT = document.getElementById("version-input"); var LINK\_TEMPLATES = \[ \['macOS', \[ \[3000, '/Sublime Text Build VERSION.dmg'\], \[4000, '/sublime\_text\_build\_VERSION\_mac.zip'\], \]\], \['Windows 32bit installer', \[ \[3000, '/Sublime Text Build VERSION Setup.exe'\], \[4000, '/sublime\_text\_build\_VERSION\_x32\_setup.exe'\] \]\], \['Windows 32bit portable zip', \[ \[3000, '/Sublime Text Build VERSION.zip'\], \[4000, '/sublime\_text\_build\_VERSION\_x32.zip'\], \]\], \['Windows 64bit installer', \[ \[3000, '/Sublime Text Build VERSION x64 Setup.exe'\], \[4000, '/sublime\_text\_build\_VERSION\_x64\_setup.exe'\], \]\], \['Windows 64bit portable zip', \[ \[3000, '/Sublime Text Build VERSION x64.zip'\], \[4000, '/sublime\_text\_build\_VERSION\_x64.zip'\], \]\], \['Linux 64bit deb package', \[ \[3000, '/sublime-text\_build-VERSION\_amd64.deb'\], \[4000, '/sublime-text\_build-VERSION\_amd64.deb'\], \]\], \['Linux 64bit rpm package', \[ \[3000, '/sublime-text-VERSION-1.x86\_64.rpm'\], \[4000, '/sublime-text-VERSION-1.x86\_64.rpm'\], \]\], \['Linux 64bit pkg package', \[ \[3000, '/sublime-text-VERSION-1-x86\_64.pkg.tar.xz'\], \[4000, '/sublime-text-VERSION-1-x86\_64.pkg.tar.xz'\], \]\], \['Linux 32bit tar archive', \[ \[3000, '/sublime\_text\_3\_build\_VERSION\_x32.tar.bz2'\], \[4000, null\], \]\], \['Linux 64bit tar archive', \[ \[3000, '/sublime\_text\_3\_build\_VERSION\_x64.tar.bz2'\], \[4000, '/sublime\_text\_build\_VERSION\_x64.tar.xz'\], \]\], \['Linux arm64 deb package', \[ \[4000, '/sublime-text\_build-VERSION\_arm64.deb'\], \]\], \['Linux arm64 pkg package', \[ \[4000, '/sublime-text-VERSION-1-aarch64.pkg.tar.xz'\], \]\], \['Linux arm64 tar archive', \[ \[4000, '/sublime\_text\_build\_VERSION\_arm64.tar.xz'\], \]\], \]; function generate\_links() { var version = VERSION\_INPUT\_ELEMENT.valueAsNumber; var urls = \[\]; for (var i = 0; i < LINK\_TEMPLATES.length; i++) { var url = null; for (var j = 0; j < LINK\_TEMPLATES\[i\]\[1\].length; ++j) { if (LINK\_TEMPLATES\[i\]\[1\]\[j\]\[0\] > version) break; url = LINK\_TEMPLATES\[i\]\[1\]\[j\]\[1\]; } if (url !== null) { urls.push(\[LINK\_TEMPLATES\[i\]\[0\], BASE\_URL + url.replace('VERSION', version)\]); } } if (urls.length > 0) { LINK\_LIST\_ELEMENT.textContent = ''; for (var i = 0; i < urls.length; ++i) { var li = document.createElement('li'); var link = document.createElement('a'); link.href = urls\[i\]\[1\]; link.textContent = urls\[i\]\[0\]; li.appendChild(link); LINK\_LIST\_ELEMENT.appendChild(li); } } else { LINK\_LIST\_ELEMENT.textContent = 'Invalid build number'; } } (function() { var build = VERSION\_INPUT\_ELEMENT.value; var params = new URLSearchParams(window.location.search); var param\_build = params.get('build'); if (param\_build) build = param\_build; if (build) { VERSION\_INPUT\_ELEMENT.value = build; generate\_links(); } })();
...
"popup\_css"[🔗](){id=doc_popup_css}

CSS passed to popups.

"phantom\_css"[🔗](){id=doc_phantom_css}

CSS passed to phantoms. _If not specified, uses `popup_css`._

"sheet\_css"[🔗](){id=doc_sheet_css}
...
"gutter\_foreground"[🔗](){id=doc_gutter_foreground}

The color of line numbers in the gutter.

"gutter\_foreground\_highlight"[🔗](){id=doc_gutter_foreground_highlight}
...
"line\_diff\_width"[🔗](){id=doc_line_diff_width}

3186

The width of the diff lines, between `1` and `8`.

"line\_diff\_added"[🔗](){id=doc_line_diff_added}

3189

The color of diff markers for added lines.

"line\_diff\_modified"[🔗](){id=doc_line_diff_modified}

3186

The color of diff markers for modified lines.

"line\_diff\_deleted"[🔗](){id=doc_line_diff_deleted}
...
"find\_highlight"[🔗](){id=doc_find_highlight}

The background color of text matched by the Find panel.

"find\_highlight\_foreground"[🔗](){id=doc_find_highlight_foreground}

A color that will override the scope-based text color.

"scroll\_highlight"[🔗](){id=doc_scroll_highlight}

4050

The color search result positions drawn on top of the scroll bar.\*

"scroll\_selected\_highlight"[🔗](){id=doc_scroll_selected_highlight}
...
"active\_guide"[🔗](){id=doc_active_guide}
...
"stack\_guide"[🔗](){id=doc_stack_guide}
...
"brackets\_options"[🔗](){id=doc_brackets_options}
...
"brackets\_foreground"[🔗](){id=doc_brackets_foreground}

The color to use when drawing the style specified by `brackets_options`.

"bracket\_contents\_options"[🔗](){id=doc_bracket_contents_options}
...
"bracket\_contents\_foreground"[🔗](){id=doc_bracket_contents_foreground}
...
"tags\_options"[🔗](){id=doc_tags_options}
...
"tags\_foreground"[🔗](){id=doc_tags_foreground}
...
"shadow\_width"[🔗](){id=doc_shadow_width}
...
"foreground\_adjust"[🔗](){id=doc_foreground_adjust}
...
section#css\_colors-section:after { content: ""; display: table; clear: both; } span.color-block { width: 50px; height: 20px; display: inline-block; position: relative;top: 5px; } div.col { float: left; width: 33%; }
...
\[
    // Set up the textures for a button
    {
        ...
        "layer0.tint": "#000",
        "layer0.opacity": 1.0,
        "layer1.texture": "Theme \- Example/textures/button\_background.png",
        "layer1.inner\_margin": 4,
        "layer1.opacity": 1.0,
        "layer2.texture": "Theme \- Example/textures/button\_highlight.png",
        "layer2.inner\_margin": 4,
        "layer2.opacity": 0.0,
        "content\_margin": \[4, 8, 4, 8\]
    },
    ...
    {
        "class": "label\_control",
        "parents": \[{"class": "button\_control", "attributes": \["hover"\]}\],
        "fg": \[255, 255, 255\]
    }
\]
...
"overlay\_scroll\_bars"[🔗](){id=doc_overlay_scroll_bars}
...
"highlight\_modified\_tabs"[🔗](){id=doc_highlight_modified_tabs}

If the tabs of modified files should be highlighted. This setting should be checked in addition to the `dirty` attribute.

"show\_tab\_close\_buttons"[🔗](){id=doc_show_tab_close_buttons}

If tabs should have close buttons.

"inactive\_sheet\_dimming"[🔗](){id=doc_inactive_sheet_dimming}
...
\[
    {
        "class": "sidebar\_heading",
        "font.size": 15,
    },
    {
        "class": "sidebar\_label",
        "font.size": 14
    }
\]
...
\[
    {
        ...
        "children":
        \[
            { "command": "new\_file", "caption": "New File", "mnemonic": "N" },

            { "command": "prompt\_open\_file", "caption": "Open File…", "mnemonic": "O", "platform": "!OSX" },
            { "command": "prompt\_open\_folder", "caption": "Open Folder…", "platform": "!OSX" },
            { "command": "prompt\_open", "caption": "Open…", "platform": "OSX" }
        \]
    }
\]
...
Sublime Text runs plugins in a separate process from the main editor UI. This process runs an executable named plugin\_host.

Running plugins in a separate process ensures the entire editor will not crash due to a poorly written plugin. If a plugin does cause the plugin\_host to crash, a user may still save their work before re-starting Sublime Text.

All plugins are run in a single plugin\_host process, and share a single Python environment. Each plugin is loaded as a sub-module of a module named after the package. For example, a plugin in the file MyPackage/my\_plugin.py will be loaded as the Python module `MyPackage.my_plugin`.

The plugin\_host process contains an embedded version of the Python programming language, and exposes an [API](api_reference.html) to plugins. The plugin\_host executable always uses its own embedded version of Python, even if the end-user has Python installed on their machine.
...
The Python environment within plugin\_host contains all of the modules in [The Python Standard Library](https://docs.python.org/3.3/library/), except for:
...
*   [Types](#types)
*   [`sublime`](#module-sublime "sublime")
*   [`Window`][Window]
*   [`Edit`][Edit]
*   [`Region`][Region]
...
*   [`sublime_plugin`](#module-sublime_plugin "sublime_plugin")
*   [`CommandInputHandler`][CommandInputHandler]
*   [`BackInputHandler`][BackInputHandler]
...
1.  sublime.DIP _= [float][py3_float]_ 
    [🔗](){id=sublime_DIP_}

    Represents a device-independent pixel position.


2.  sublime.Vector _= [tuple][py3_tuple]\[[DIP][DIP], [DIP][DIP]\]_ 
    [🔗](){id=sublime_Vector_}

    Represents a X and Y coordinate.


3.  sublime.Point _= [int][py3_int]_ 
    [🔗](){id=sublime_Point_}

    Represents the offset from the beginning of the editor buffer.


4.  sublime.Value _= [bool][py3_bool] | [str][py3_str] | [int][py3_int] | [float][py3_float] | [list][py3_list]\[[Value][Value]\] | [dict][py3_dict]\[[str][py3_str], [Value][Value]\] | [None][py3_None]_ 
    [🔗](){id=sublime_Value_}

    A JSON-equivalent value.


5.  sublime.CommandArgs _= [dict][py3_dict]\[[str][py3_str], [Value][Value]\] | [None][py3_None]_ 
    [🔗](){id=sublime_CommandArgs_}

    The arguments to a command may be `None` or a `dict` of `str` keys.


6.  sublime.Kind _= [tuple][py3_tuple]\[[KindId][KindId], [str][py3_str], [str][py3_str]\]_ 
    [🔗](){id=sublime_Kind_}

    Metadata about the kind of a symbol, [`CompletionItem`][CompletionItem], [`QuickPanelItem`][QuickPanelItem] or [`ListInputItem`][ListInputItem]. Controls the color and letter shown in the “icon” presented to the left of the item.


7.  sublime.Event _= [dict][py3_dict]_ 
    [🔗](){id=sublime_Event_}
    ...
    *   [`ListInputHandler`][ListInputHandler] classes may opt-in to receive a second arg to the [`validate()`][validate] and [`confirm()`][confirm] methods by by implementing the method [`want_event()`][want_event] and returning `True`. 4096
    ...
    Present when the user selects an item from a quick panel, selects an item from a [`ListInputHandler`][ListInputHandler], or clicks a link in a [minihtml][minihtml] document.


8.  sublime.CompletionValue _= [str][py3_str] | [tuple][py3_tuple]\[[str][py3_str], [str][py3_str]\] | [CompletionItem][CompletionItem]_ 
    [🔗](){id=sublime_CompletionValue_}
...
1.  Bases: [`IntFlag`][py3_enum_IntFlag]
    ...
    See [`Window.new_html_sheet`][new_html_sheet], [`Window.new_file`][new_file] and [`Window.open_file`][open_file].

    For backwards compatibility these values are also available outside this enumeration (without a prefix).


2.  NONE _= 0_ 
    [🔗](){id=sublime_NewFileFlags_NONE_}


3.  ENCODED_POSITION _= 1_ 
    [🔗](){id=sublime_NewFileFlags_ENCODED_POSITION_}
...
4.  TRANSIENT _= 4_ 
    [🔗](){id=sublime_NewFileFlags_TRANSIENT_}
...
5.  FORCE_GROUP _= 8_ 
    [🔗](){id=sublime_NewFileFlags_FORCE_GROUP_}
...
10.  FORCE_CLONE _= 256_ 
    [🔗](){id=sublime_NewFileFlags_FORCE_CLONE_}
...
1.  Bases: [`IntFlag`][py3_enum_IntFlag]
    ...
    Flags for use when searching through a [`View`][View].

    See [`View.find`][find] and [`View.find_all`][find_all].

    For backwards compatibility these values are also available outside this enumeration (without a prefix).


2.  NONE _= 0_ 
    [🔗](){id=sublime_FindFlags_NONE_}


3.  LITERAL _= 1_ 
    [🔗](){id=sublime_FindFlags_LITERAL_}

    Whether the find pattern should be matched literally or as a regex.


4.  IGNORECASE _= 2_ 
    [🔗](){id=sublime_FindFlags_IGNORECASE_}
...
1.  Bases: [`IntFlag`][py3_enum_IntFlag]
    ...
    See [`Window.show_quick_panel`][show_quick_panel].

    For backwards compatibility these values are also available outside this enumeration (without a prefix).


2.  NONE _= 0_ 
    [🔗](){id=sublime_QuickPanelFlags_NONE_}


3.  MONOSPACE_FONT _= 1_ 
    [🔗](){id=sublime_QuickPanelFlags_MONOSPACE_FONT_}

    Use a monospace font.


4.  KEEP_OPEN_ON_FOCUS_LOST _= 2_ 
    [🔗](){id=sublime_QuickPanelFlags_KEEP_OPEN_ON_FOCUS_LOST_}
...
1.  Bases: [`IntFlag`][py3_enum_IntFlag]
    ...
    See [`View.show_popup`][show_popup].

    For backwards compatibility these values are also available outside this enumeration (without a prefix).


2.  NONE _= 0_ 
    [🔗](){id=sublime_PopupFlags_NONE_}


3.  COOPERATE_WITH_AUTO_COMPLETE _= 2_ 
    [🔗](){id=sublime_PopupFlags_COOPERATE_WITH_AUTO_COMPLETE_}

    Causes the popup to display next to the auto complete menu.


4.  HIDE_ON_MOUSE_MOVE _= 4_ 
    [🔗](){id=sublime_PopupFlags_HIDE_ON_MOUSE_MOVE_}

    Causes the popup to hide when the mouse is moved, clicked or scrolled.


5.  HIDE_ON_MOUSE_MOVE_AWAY _= 8_ 
    [🔗](){id=sublime_PopupFlags_HIDE_ON_MOUSE_MOVE_AWAY_}
...
01. Bases: [`IntFlag`][py3_enum_IntFlag]
    ...
    Flags for use with added regions. See [`View.add_regions`][add_regions].

    For backwards compatibility these values are also available outside this enumeration (without a prefix).


02. NONE _= 0_ 
    [🔗](){id=sublime_RegionFlags_NONE_}


03. DRAW_EMPTY _= 1_ 
    [🔗](){id=sublime_RegionFlags_DRAW_EMPTY_}

    Draw empty regions with a vertical bar. By default, they aren’t drawn at all.


04. HIDE_ON_MINIMAP _= 2_ 
    [🔗](){id=sublime_RegionFlags_HIDE_ON_MINIMAP_}

    Don’t show the regions on the minimap.


05. DRAW_EMPTY_AS_OVERWRITE _= 4_ 
    [🔗](){id=sublime_RegionFlags_DRAW_EMPTY_AS_OVERWRITE_}

    Draw empty regions with a horizontal bar instead of a vertical one.


06. PERSISTENT _= 16_ 
    [🔗](){id=sublime_RegionFlags_PERSISTENT_}

    Save the regions in the session.


07. DRAW_NO_FILL _= 32_ 
    [🔗](){id=sublime_RegionFlags_DRAW_NO_FILL_}

    Disable filling the regions, leaving only the outline.


08. HIDDEN _= 128_ 
    [🔗](){id=sublime_RegionFlags_HIDDEN_}

    Don’t draw the regions.


09. DRAW_NO_OUTLINE _= 256_ 
    [🔗](){id=sublime_RegionFlags_DRAW_NO_OUTLINE_}

    Disable drawing the outline of the regions.


10. DRAW_SOLID_UNDERLINE _= 512_ 
    [🔗](){id=sublime_RegionFlags_DRAW_SOLID_UNDERLINE_}

    Draw a solid underline below the regions.


11. DRAW_STIPPLED_UNDERLINE _= 1024_ 
    [🔗](){id=sublime_RegionFlags_DRAW_STIPPLED_UNDERLINE_}

    Draw a stippled underline below the regions.


12. DRAW_SQUIGGLY_UNDERLINE _= 2048_ 
    [🔗](){id=sublime_RegionFlags_DRAW_SQUIGGLY_UNDERLINE_}

    Draw a squiggly underline below the regions.


13. NO_UNDO _= 8192_ 
    [🔗](){id=sublime_RegionFlags_NO_UNDO_}
...
1.  Bases: [`IntEnum`][py3_enum_IntEnum]
    ...
    See [`EventListener.on_query_context`][on_query_context] and [`ViewEventListener.on_query_context`][on_query_context].

    For backwards compatibility these values are also available outside this enumeration with a `OP_` prefix.


2.  EQUAL _= 0_ 
    [🔗](){id=sublime_QueryOperator_EQUAL_}


3.  NOT_EQUAL _= 1_ 
    [🔗](){id=sublime_QueryOperator_NOT_EQUAL_}


4.  REGEX_MATCH _= 2_ 
    [🔗](){id=sublime_QueryOperator_REGEX_MATCH_}


5.  NOT_REGEX_MATCH _= 3_ 
    [🔗](){id=sublime_QueryOperator_NOT_REGEX_MATCH_}


6.  REGEX_CONTAINS _= 4_ 
    [🔗](){id=sublime_QueryOperator_REGEX_CONTAINS_}


7.  NOT_REGEX_CONTAINS _= 5_ 
    [🔗](){id=sublime_QueryOperator_NOT_REGEX_CONTAINS_}
...
01. Bases: [`IntFlag`][py3_enum_IntFlag]
    ...
    Flags that identify characteristics about a [`Point`][Point] in a text sheet. See [`View.classify`][classify].

    For backwards compatibility these values are also available outside this enumeration with a `CLASS_` prefix.


02. NONE _= 0_ 
    [🔗](){id=sublime_PointClassification_NONE_}


03. WORD_START _= 1_ 
    [🔗](){id=sublime_PointClassification_WORD_START_}

    The point is the start of a word.


04. WORD_END _= 2_ 
    [🔗](){id=sublime_PointClassification_WORD_END_}

    The point is the end of a word.


05. PUNCTUATION_START _= 4_ 
    [🔗](){id=sublime_PointClassification_PUNCTUATION_START_}

    The point is the start of a sequence of punctuation characters.


06. PUNCTUATION_END _= 8_ 
    [🔗](){id=sublime_PointClassification_PUNCTUATION_END_}

    The point is the end of a sequence of punctuation characters.


07. SUB_WORD_START _= 16_ 
    [🔗](){id=sublime_PointClassification_SUB_WORD_START_}

    The point is the start of a sub-word.


08. SUB_WORD_END _= 32_ 
    [🔗](){id=sublime_PointClassification_SUB_WORD_END_}

    The point is the end of a sub-word.


09. LINE_START _= 64_ 
    [🔗](){id=sublime_PointClassification_LINE_START_}

    The point is the start of a line.


10.  LINE_END _= 128_ 
    [🔗](){id=sublime_PointClassification_LINE_END_}

    The point is the end of a line.


11.  EMPTY_LINE _= 256_ 
    [🔗](){id=sublime_PointClassification_EMPTY_LINE_}
...
1.  Bases: [`IntFlag`][py3_enum_IntFlag]
    ...
    Flags controlling how asynchronous completions function. See [`CompletionList`][CompletionList].

    For backwards compatibility these values are also available outside this enumeration (without a prefix).

2.  NONE _= 0_ 
    [🔗](){id=sublime_AutoCompleteFlags_NONE_}

3.  INHIBIT_WORD_COMPLETIONS _= 8_ 
    [🔗](){id=sublime_AutoCompleteFlags_INHIBIT_WORD_COMPLETIONS_}

    Prevent Sublime Text from showing completions based on the contents of the view.

4.  INHIBIT_EXPLICIT_COMPLETIONS _= 16_ 
    [🔗](){id=sublime_AutoCompleteFlags_INHIBIT_EXPLICIT_COMPLETIONS_}
...
1.  Bases: [`IntEnum`][py3_enum_IntEnum]
    ...
    The result from a _yes / no / cancel_ dialog. See [`yes_no_cancel_dialog`][yes_no_cancel_dialog].

    For backwards compatibility these values are also available outside this enumeration with a `DIALOG_` prefix.

2.  CANCEL _= 0_ 
    [🔗](){id=sublime_DialogResult_CANCEL_}

3.  YES _= 1_ 
    [🔗](){id=sublime_DialogResult_YES_}

4.  NO _= 2_ 
    [🔗](){id=sublime_DialogResult_NO_}
...
1.  Bases: [`IntEnum`][py3_enum_IntEnum]
    ...
    How a [`Phantom`][Phantom] should be positioned. See [`PhantomSet`][PhantomSet].

    For backwards compatibility these values are also available outside this enumeration with a `LAYOUT_` prefix.

2.  INLINE _= 0_ 
    [🔗](){id=sublime_PhantomLayout_INLINE_}

    The phantom is positioned inline with the text at the beginning of its [`Region`][Region].

3.  BELOW _= 1_ 
    [🔗](){id=sublime_PhantomLayout_BELOW_}

    The phantom is positioned below the line, left-aligned with the beginning of its [`Region`][Region].

4.  BLOCK _= 2_ 
    [🔗](){id=sublime_PhantomLayout_BLOCK_}
...
02. AMBIGUOUS _= 0_ 
    [🔗](){id=sublime_KindId_AMBIGUOUS_}

03. KEYWORD _= 1_ 
    [🔗](){id=sublime_KindId_KEYWORD_}

04. TYPE _= 2_ 
    [🔗](){id=sublime_KindId_TYPE_}

05. FUNCTION _= 3_ 
    [🔗](){id=sublime_KindId_FUNCTION_}

06. NAMESPACE _= 4_ 
    [🔗](){id=sublime_KindId_NAMESPACE_}

07. NAVIGATION _= 5_ 
    [🔗](){id=sublime_KindId_NAVIGATION_}

08. MARKUP _= 6_ 
    [🔗](){id=sublime_KindId_MARKUP_}

09. VARIABLE _= 7_ 
    [🔗](){id=sublime_KindId_VARIABLE_}

10. SNIPPET _= 8_ 
    [🔗](){id=sublime_KindId_SNIPPET_}

11. COLOR_REDISH _= 9_ 
    [🔗](){id=sublime_KindId_COLOR_REDISH_}

12. COLOR_ORANGISH _= 10_ 
    [🔗](){id=sublime_KindId_COLOR_ORANGISH_}

13. COLOR_YELLOWISH _= 11_ 
    [🔗](){id=sublime_KindId_COLOR_YELLOWISH_}

14. COLOR_GREENISH _= 12_ 
    [🔗](){id=sublime_KindId_COLOR_GREENISH_}

15. COLOR_CYANISH _= 13_ 
    [🔗](){id=sublime_KindId_COLOR_CYANISH_}

16. COLOR_BLUISH _= 14_ 
    [🔗](){id=sublime_KindId_COLOR_BLUISH_}

17. COLOR_PURPLISH _= 15_ 
    [🔗](){id=sublime_KindId_COLOR_PURPLISH_}

18. COLOR_PINKISH _= 16_ 
    [🔗](){id=sublime_KindId_COLOR_PINKISH_}

19. COLOR_DARK _= 17_ 
    [🔗](){id=sublime_KindId_COLOR_DARK_}
...
1.  Bases: [`IntEnum`][py3_enum_IntEnum]
    ...
    See [`Window.symbol_locations`][symbol_locations].
...
1.  Bases: [`IntEnum`][py3_enum_IntEnum]
    ...
    See [`Window.symbol_locations`][symbol_locations] and [`View.indexed_symbol_regions`][indexed_symbol_regions].
...
01. Bases: [`IntEnum`][py3_enum_IntEnum]
    ...
    The format completion text can be in. See [`CompletionItem`][CompletionItem].
...
05. sublime.version() → [str][py3_str] 
    [🔗](){id=sublime_version_}
...
06. sublime.platform() → [Literal][py3_typing_Literal]\['osx', 'linux', 'windows'\] 
    [🔗](){id=sublime_platform_}
...
07. sublime.arch() → [Literal][py3_typing_Literal]\['x32', 'x64', 'arm64'\] 
    [🔗](){id=sublime_arch_}
...
08. sublime.channel() → [Literal][py3_typing_Literal]\['dev', 'stable'\] 
    [🔗](){id=sublime_channel_}
...
11. sublime.packages_path() → [str][py3_str] 
...
14. sublime.status_message(_msg: [str][py3_str]_) 
    [🔗](){id=sublime_status_message_}


    Show a message in the status bar.

15. sublime.error_message(_msg: [str][py3_str]_) 
    [🔗](){id=sublime_error_message_}


    Display an error dialog.

16. sublime.message_dialog(_msg: [str][py3_str]_) 
    [🔗](){id=sublime_message_dialog_}


    Display a message dialog.

17. sublime.ok_cancel_dialog(_msg: [str][py3_str]_, _ok_title=''_, _title=''_) → [bool][py3_bool] 
    [🔗](){id=sublime_ok_cancel_dialog_}


    Show a _ok / cancel_ question dialog.
    ...
    Whether the user pressed the _ok_ button.

18. sublime.yes_no_cancel_dialog(_msg: [str][py3_str]_, _yes_title=''_, _no_title=''_, _title=''_) → [DialogResult][DialogResult] 
    [🔗](){id=sublime_yes_no_cancel_dialog_}


    Show a _yes / no / cancel_ question dialog.
    ...
    Text to display on the _yes_ button.
...
22. sublime.choose_font_dialog(_callback: Callable\[\[[Value][Value]\], [None][py3_None]\]_, _default: [Value][Value] = None_)  4157
    ...
    The default values to select/return. Same format as the argument passed to [`callback`][py3_term-callback].

23. sublime.run_command(_cmd: [str][py3_str]_, _args: [CommandArgs][CommandArgs] = None_) 
    [🔗](){id=sublime_run_command_}
...
28. sublime.get_clipboard(_size_limit: [int][py3_int] = 16777216_) → [str][py3_str] 
    [🔗](){id=sublime_get_clipboard_}
...
30. sublime.log_commands(_flag: [bool][py3_bool] | [None][py3_None] = None_) 
    [🔗](){id=sublime_log_commands_}
...
32. sublime.log_input(_flag: [bool][py3_bool] | [None][py3_None] = None_) 
    [🔗](){id=sublime_log_input_}
...
36. sublime.log_result_regex(_flag: [bool][py3_bool] | [None][py3_None] = None_) 
    [🔗](){id=sublime_log_result_regex_}
...
38. sublime.log_indexing(_flag: [bool][py3_bool] | [None][py3_None] = None_) 
    [🔗](){id=sublime_log_indexing_}
...
40. sublime.log_build_systems(_flag: [bool][py3_bool] | [None][py3_None] = None_) 
    [🔗](){id=sublime_log_build_systems_}
...
45. sublime.score_selector(_scope_name: [str][py3_str]_, _selector: [str][py3_str]_) → [int][py3_int] 
    [🔗](){id=sublime_score_selector_}
...
46. sublime.load_resource(_name: [str][py3_str]_) → [str][py3_str] 
    [🔗](){id=sublime_load_resource_}


    Loads the given resource. The name should be in the format “Packages/Default/Main.sublime-menu”.

    Raises [`FileNotFoundError`][py3_FileNotFoundError]:

    if resource is not found

47. sublime.load_binary_resource(_name_) → [bytes][py3_bytes] 
    [🔗](){id=sublime_load_binary_resource_}
    ...
    if resource is not found"py function">

48. sublime.find_resources(_pattern: [str][py3_str]_) → [list][py3_list]\[[str][py3_str]\] 
    [🔗](){id=sublime_find_resources_}


    Finds resources whose file name matches the given glob pattern.

49. sublime.encode_value(_value: [Value][Value]_, _pretty=False_, _update_text: [str][py3_str] = None_) → [str][py3_str] 
    [🔗](){id=sublime_encode_value_}


    Encode a JSON compatible [`Value`][Value] into a string representation.
...
50. sublime.decode_value(_data: [str][py3_str]_) → [Value][Value] 
    [🔗](){id=sublime_decode_value_}


    Decode a JSON string into an object. Note that comments and trailing commas are allowed.

    Raises [`ValueError`][py3_ValueError]:

    If the string is not valid JSON.

51. sublime.expand_variables(_value: [Value][Value]_, _variables: [dict][py3_dict]\[[str][py3_str], [str][py3_str]\]_) → [Value][Value] 
    [🔗](){id=sublime_expand_variables_}
...
52. sublime.load_settings(_base_name: [str][py3_str]_) → [Settings][Settings] 
    [🔗](){id=sublime_load_settings_}
...
53. sublime.save_settings(_base_name: [str][py3_str]_) 
    [🔗](){id=sublime_save_settings_}


    Flush any in-memory changes to the named settings object to disk.

54. sublime.set_timeout(_callback: [Callable][py3_typing_Callable]_, _delay: [int][py3_int] = 0_) 
    [🔗](){id=sublime_set_timeout_}
...
55. sublime.set_timeout_async(_callback: [Callable][py3_typing_Callable]_, _delay: [int][py3_int] = 0_) 
    [🔗](){id=sublime_set_timeout_async_}


    Runs the callback on an alternate thread after the given delay (in milliseconds).

56. sublime.active_window() → [Window][Window] 
    [🔗](){id=sublime_active_window_}


    Returns:

    The most recently used [`Window`][Window].

57. sublime.windows() → [list][py3_list]\[[sublime.Window][Window]\] 
    [🔗](){id=sublime_windows_}
...
58. sublime.get_macro() → [list][py3_list]\[[dict][py3_dict]\] 
    [🔗](){id=sublime_get_macro_}
...
[🔗](){id=sublime_Window_}
_______________________________________________________________________________
## _class_ sublime.Window 
_______________________________________________________________________________

01. Bases: [`object`][py3_object]

02. id() → [int][py3_int] 
    [🔗](){id=sublime_Window_id_}

    Returns:

    A number that uniquely identifies this window.

03. is_valid() → [bool][py3_bool] 
    [🔗](){id=sublime_Window_is_valid_}

    Check whether this window is still valid. Will return `False` for a closed window, for example.

04. hwnd() → [int][py3_int] 
    [🔗](){id=sublime_Window_hwnd_}

    Returns:

    A platform specific window handle. Windows only.

05. active_sheet() → [Sheet][Sheet] | [None][py3_None] 
    [🔗](){id=sublime_Window_active_sheet_}

    Returns:

    The currently focused [`Sheet`][Sheet].

06. active_view() → [View][View] | [None][py3_None] 
    [🔗](){id=sublime_Window_active_view_}
...
07. new_html_sheet(_name: [str][py3_str]_, _contents: [str][py3_str]_, _flags=NewFileFlags.NONE_, _group=\-1_) → [Sheet][Sheet]  4065
    ...
    Only [`NewFileFlags.TRANSIENT`][TRANSIENT] and [`NewFileFlags.ADD_TO_SELECTION`][ADD_TO_SELECTION] are allowed.

    `group`

    The group to add the sheet to. `-1` for the active group.

08. run_command(_cmd: [str][py3_str]_, _args: [CommandArgs][CommandArgs] = None_) 
    [🔗](){id=sublime_Window_run_command_}

    Run the named [`WindowCommand`][WindowCommand] with the (optional) given args. This method is able to run any sort of command, dispatching the command via input focus.

09. new_file(_flags=NewFileFlags.NONE_, _syntax=''_) → [View][View] 
    [🔗](){id=sublime_Window_new_file_}
    ...
    Either `0`, [`NewFileFlags.TRANSIENT`][TRANSIENT] or [`NewFileFlags.ADD_TO_SELECTION`][ADD_TO_SELECTION].
...
11. find_open_file(_fname: [str][py3_str]_, _group=\-1_) → [View][View] | [None][py3_None] 
    ...
    The [`View`][View] to the file or `None` if the file isn’t open.

12. file_history() → [list][py3_list]\[[str][py3_str]\] 
    [🔗](){id=sublime_Window_file_history_}

    Get the list of previously opened files. This is the same list as _File > Open Recent_.

13. num_groups() → [int][py3_int] 
    [🔗](){id=sublime_Window_num_groups_}

    Returns:

    The number of view groups in the window.

14. active_group() → [int][py3_int] 
    [🔗](){id=sublime_Window_active_group_}

    Returns:

    The index of the currently selected group.

15. focus_group(_idx: [int][py3_int]_) 
    [🔗](){id=sublime_Window_focus_group_}

    Focus the specified group, making it active.

16. focus_sheet(_sheet: [Sheet][Sheet]_) 
    [🔗](){id=sublime_Window_focus_sheet_}

    Switches to the given [`Sheet`][Sheet].

17. focus_view(_view: [View][View]_) 
    [🔗](){id=sublime_Window_focus_view_}
...
19. bring_to_front() 
    [🔗](){id=sublime_Window_bring_to_front_}

    Bring the window in front of any other windows.

20. get_sheet_index(_sheet: [Sheet][Sheet]_) → [tuple][py3_tuple]\[[int][py3_int], [int][py3_int]\] 
    [🔗](){id=sublime_Window_get_sheet_index_}

    Returns:

    The a tuple of the group and index within the group of the given [`Sheet`][Sheet].

21. get_view_index(_view: [View][View]_) → [tuple][py3_tuple]\[[int][py3_int], [int][py3_int]\] 
    [🔗](){id=sublime_Window_get_view_index_}

    Returns:

    The a tuple of the group and index within the group of the given [`View`][View].

22. set_sheet_index(_sheet: [Sheet][Sheet]_, _group: [int][py3_int]_, _index: [int][py3_int]_) 
    [🔗](){id=sublime_Window_set_sheet_index_}

    Move the given [`Sheet`][Sheet] to the given `group` at the given `index`.

23. set_view_index(_view: [View][View]_, _group: [int][py3_int]_, _index: [int][py3_int]_) 
    [🔗](){id=sublime_Window_set_view_index_}
...
25. sheets() → [list][py3_list]\[[sublime.Sheet][Sheet]\] 
    [🔗](){id=sublime_Window_sheets_}

    Returns:

    All open sheets in the window.

26. views(_\*_, _include_transient=False_) → [list][py3_list]\[[sublime.View][View]\] 
    [🔗](){id=sublime_Window_views_}
...
29. active_sheet_in_group(_group: [int][py3_int]_) → [Sheet][Sheet] | [None][py3_None] 
    [🔗](){id=sublime_Window_active_sheet_in_group_}

    Returns:

    The currently focused [`Sheet`][Sheet] in the given group.

30. active_view_in_group(_group: [int][py3_int]_) → [View][View] | [None][py3_None] 
    [🔗](){id=sublime_Window_active_view_in_group_}

    Returns:

    The currently focused [`View`][View] in the given group.

31. sheets_in_group(_group: [int][py3_int]_) → [list][py3_list]\[[sublime.Sheet][Sheet]\] 
    [🔗](){id=sublime_Window_sheets_in_group_}

    Returns:

    A list of all sheets in the specified group.

32. views_in_group(_group: [int][py3_int]_) → [list][py3_list]\[[sublime.View][View]\] 
    [🔗](){id=sublime_Window_views_in_group_}

    Returns:

    A list of all views in the specified group.

33. num_sheets_in_group(_group: [int][py3_int]_) → [int][py3_int] 
    [🔗](){id=sublime_Window_num_sheets_in_group_}

    Returns:

34. The number of sheets in the specified group.bject py" id="sublime.Window.num_views_in_group"> num_views_in_group(_group: [int][py3_int]_) → [int][py3_int] 
    [🔗](){id=sublime_Window_num_views_in_group_}

    Returns:

    The number of views in the specified group.

35. transient_sheet_in_group(_group: [int][py3_int]_) → [Sheet][Sheet] | [None][py3_None] 
    [🔗](){id=sublime_Window_transient_sheet_in_group_}

    Returns:

    The transient sheet in the specified group.

36. transient_view_in_group(_group: [int][py3_int]_) → [View][View] | [None][py3_None] 
    [🔗](){id=sublime_Window_transient_view_in_group_}

    Returns:

    The transient view in the specified group.

37. promote_sheet(_sheet: [Sheet][Sheet]_) 
    [🔗](){id=sublime_Window_promote_sheet_}
...
39. get_layout() 
    [🔗](){id=sublime_Window_get_layout_}

    Deprecated:

    Use [`layout()`][layout] instead

40. set_layout(_layout: [dict][py3_dict]\[[str][py3_str], [Value][Value]\]_) 
    [🔗](){id=sublime_Window_set_layout_}

    Set the group layout of the window.

41. create_output_panel(_name: [str][py3_str]_, _unlisted=False_) → [View][View] 
    [🔗](){id=sublime_Window_create_output_panel_}
...
42. find_output_panel(_name: [str][py3_str]_) → [View][View] | [None][py3_None] 
    [🔗](){id=sublime_Window_find_output_panel_}

    Returns:

    The [`View`][View] associated with the named output panel, or `None` if the output panel does not exist.

43. destroy_output_panel(_name: [str][py3_str]_) 
    [🔗](){id=sublime_Window_destroy_output_panel_}

    Destroy the named output panel, hiding it if currently open.

44. active_panel() → [str][py3_str] | [None][py3_None] 
    [🔗](){id=sublime_Window_active_panel_}
...
45. panels() → [list][py3_list]\[[str][py3_str]\] 
    [🔗](){id=sublime_Window_panels_}
...
46. get_output_panel(_name: [str][py3_str]_) 
    [🔗](){id=sublime_Window_get_output_panel_}

    Deprecated:

    Use [`create_output_panel`][create_output_panel] instead.

47. show_input_panel(_caption: [str][py3_str]_, _initial_text: [str][py3_str]_, _on_done: [Callable][py3_typing_Callable]\[\[[str][py3_str]\], [None][py3_None]\] | [None][py3_None]_, _on_change: [Callable][py3_typing_Callable]\[\[[str][py3_str]\], [None][py3_None]\] | [None][py3_None]_, _on_cancel: [Callable][py3_typing_Callable]\[\[\], [None][py3_None]\] | [None][py3_None]_) 
    [🔗](){id=sublime_Window_show_input_panel_}
    ...
    The [`View`][View] used for the input widget.

48. show_quick_panel(_items: [list][py3_list]\[[str][py3_str]\] | [list][py3_list]\[[list][py3_list]\[[str][py3_str]\]\] | [list][py3_list]\[[sublime.QuickPanelItem][QuickPanelItem]\]_, _on_select: [Callable][py3_typing_Callable]\[\[[int][py3_int]\], [None][py3_None]\]_, _flags=QuickPanelFlags.NONE_, _selected_index=\-1_, _on_highlight: [Callable][py3_typing_Callable]\[\[[int][py3_int]\], [None][py3_None]\] | [None][py3_None] = None_, _placeholder: [str][py3_str] | [None][py3_None] = None_) 
    [🔗](){id=sublime_Window_show_quick_panel_}
    ...
    [`QuickPanelFlags`][QuickPanelFlags] controlling behavior.
...
49. is_sidebar_visible() → [bool][py3_bool] 
    [🔗](){id=sublime_Window_is_sidebar_visible_}

    Returns:

    Whether the sidebar is visible.

50. set_sidebar_visible(_flag: [bool][py3_bool]_, _animate=True_) 
    [🔗](){id=sublime_Window_set_sidebar_visible_}

    Hides or shows the sidebar.

51. is_minimap_visible() → [bool][py3_bool] 
    [🔗](){id=sublime_Window_is_minimap_visible_}

    Returns:

    Whether the minimap is visible.

52. set_minimap_visible(_flag: [bool][py3_bool]_) 
    [🔗](){id=sublime_Window_set_minimap_visible_}

    Hides or shows the minimap.

53. is_status_bar_visible() → [bool][py3_bool] 
    [🔗](){id=sublime_Window_is_status_bar_visible_}

    Returns:

    Whether the status bar is visible.

54. set_status_bar_visible(_flag: [bool][py3_bool]_) 
    [🔗](){id=sublime_Window_set_status_bar_visible_}

    Hides or shows the status bar.

55. get_tabs_visible() → [bool][py3_bool] 
    [🔗](){id=sublime_Window_get_tabs_visible_}

    Returns:

    Whether the tabs are visible.

56. set_tabs_visible(_flag: [bool][py3_bool]_) 
    [🔗](){id=sublime_Window_set_tabs_visible_}

    Hides or shows the tabs.

57. is_menu_visible() → [bool][py3_bool] 
    [🔗](){id=sublime_Window_is_menu_visible_}

    Returns:

    Whether the menu is visible.

58. set_menu_visible(_flag: [bool][py3_bool]_) 
    [🔗](){id=sublime_Window_set_menu_visible_}

    Hides or shows the menu.

59. folders() → [list][py3_list]\[[str][py3_str]\] 
    [🔗](){id=sublime_Window_folders_}

    Returns:

    A list of the currently open folders in this [`Window`][Window].

60. project_file_name() → [str][py3_str] | [None][py3_None] 
    [🔗](){id=sublime_Window_project_file_name_}
...
62. project_data() → [Value][Value] 
    [🔗](){id=sublime_Window_project_data_}
...
63. set_project_data(_data: [Value][Value]_) 
    [🔗](){id=sublime_Window_set_project_data_}
...
64. settings() → [Settings][Settings] 
    [🔗](){id=sublime_Window_settings_}

    Returns:

    The [`Settings`][Settings] object for this [`Window`][Window]. Any changes to this settings object will be specific to this window.

65. template_settings() → [Settings][Settings] 
    [🔗](){id=sublime_Window_template_settings_}
...
66. symbol_locations(_sym: [str][py3_str]_, _source=SymbolSource.ANY_, _type=SymbolType.ANY_, _kind_id=KindId.AMBIGUOUS_, _kind_letter=''_) → [list][py3_list]\[[sublime.SymbolLocation][SymbolLocation]\]  4085
    ...
    The [`KindId`][KindId] of the symbol.

    `kind_letter`

    The letter representing the kind of the symbol. See [`Kind`][Kind].

    Return:

    the found symbol locations.

67. lookup_symbol_in_index(_symbol: [str][py3_str]_) → [list][py3_list]\[[sublime.SymbolLocation][SymbolLocation]\] 
    [🔗](){id=sublime_Window_lookup_symbol_in_index_}
    ...
    Use [`symbol_locations()`][symbol_locations] instead.

68. lookup_symbol_in_open_files(_symbol: [str][py3_str]_) → [list][py3_list]\[[sublime.SymbolLocation][SymbolLocation]\] 
    [🔗](){id=sublime_Window_lookup_symbol_in_open_files_}
...
69. lookup_references_in_index(_symbol: [str][py3_str]_) → [list][py3_list]\[[sublime.SymbolLocation][SymbolLocation]\] 
    [🔗](){id=sublime_Window_lookup_references_in_index_}
...
70. lookup_references_in_open_files(_symbol: [str][py3_str]_) → [list][py3_list]\[[sublime.SymbolLocation][SymbolLocation]\] 
    [🔗](){id=sublime_Window_lookup_references_in_open_files_}
...
71. extract_variables() → [dict][py3_dict]\[[str][py3_str], [str][py3_str]\] 
    [🔗](){id=sublime_Window_extract_variables_}
    ...
    This `dict` is suitable for use with [`expand_variables()`][expand_variables].

72. status_message(_msg: [str][py3_str]_) 
    [🔗](){id=sublime_Window_status_message_}

    Show a message in the status bar.

[🔗](){id=sublime_Edit_}
_______________________________________________________________________________
## _class_ sublime.Edit 
_______________________________________________________________________________
...
1. Bases: [`object`][py3_object]
    ...
    Edit objects are passed to [`TextCommand`][TextCommand]s, and can not be created by the user. Using an invalid Edit object, or an Edit object from a different [`View`][View], will cause the functions that require them to fail.

[🔗](){id=sublime_Region_}
_______________________________________________________________________________
## _class_ sublime.Region 
_______________________________________________________________________________
...
5.  __len__() → [int][py3_int] 
    [🔗](){id=sublime_Region___len___}
...
8.  empty() → [bool][py3_bool] 
    [🔗](){id=sublime_Region_empty_}
...
11.  size() → [int][py3_int] 
    [🔗](){id=sublime_Region_size_}


    Equivalent to [`__len__`][__len__].

12.  contains(_x: [Point][Point]_) → [bool][py3_bool] 
    [🔗](){id=sublime_Region_contains_}


    Equivalent to [`__contains__`][__contains__].

13.  cover(_region: [Region][Region]_) → [Region][Region] 
    [🔗](){id=sublime_Region_cover_}


    Returns:

    A [`Region`][Region] spanning both regions.

14.  intersection(_region: [Region][Region]_) → [Region][Region] 
    [🔗](){id=sublime_Region_intersection_}


    Returns:

    A [`Region`][Region] covered by both regions.

15.  intersects(_region: [Region][Region]_) → [bool][py3_bool] 
    [🔗](){id=sublime_Region_intersects_}
...
1.  Bases: [`object`][py3_object]
    ...
    Provides a snapshot of the row and column info for a [`Point`][Point], before changes were made to a [`View`][View]. This is primarily useful for replaying changes to a document.

2.  pt _: [Point][Point]_ 
    [🔗](){id=sublime_HistoricPosition_pt_}

    The offset from the beginning of the [`View`][View].

3.  row _: [int][py3_int]_ 
    [🔗](){id=sublime_HistoricPosition_row_}

    The row the `.py` was in when the [`HistoricPosition`][HistoricPosition] was recorded.

4.  col _: [int][py3_int]_ 
    [🔗](){id=sublime_HistoricPosition_col_}
...
1.  Bases: [`object`][py3_object]
    ...
    Represents a change that occurred to the text of a [`View`][View]. This is primarily useful for replaying changes to a document.

2.  a _: [HistoricPosition][HistoricPosition]_ 
    [🔗](){id=sublime_TextChange_a_}

    The beginning [`HistoricPosition`][HistoricPosition] of the region that was modified.

3.  b _: [HistoricPosition][HistoricPosition]_ 
    [🔗](){id=sublime_TextChange_b_}
...
6.  str _: [str][py3_str]_
    [🔗](){id=sublime_TextChange_str_}

    A string of the _new_ contents of the region specified by `.a` and `.b`.

[🔗](){id=sublime_Selection_}
_______________________________________________________________________________
## _class_ sublime.Selection 
_______________________________________________________________________________
...
2.  __len__() → [int][py3_int] 
    [🔗](){id=sublime_Selection___len___}

    Returns:

    The number of regions in the selection.

3.  __delitem__(_index: [int][py3_int]_) 
    [🔗](){id=sublime_Selection___delitem___}

    Delete the region at the given `index`.

4.  is_valid() → [bool][py3_bool] 
    [🔗](){id=sublime_Selection_is_valid_}
...
5.  clear() 
    [🔗](){id=sublime_Selection_clear_}

    Remove all regions from the selection.

6.  add(_x: [Region][Region] | [Point][Point]_) 
    [🔗](){id=sublime_Selection_add_}

    Add a [`Region`][Region] or [`Point`][Point] to the selection. It will be merged with the existing regions if intersecting.

7.  add_all(_regions: Iterable\[[Region][Region] | [Point][Point]\]_) 
    [🔗](){id=sublime_Selection_add_all_}

    Add all the regions from the given iterable.

8.  subtract(_region: [Region][Region]_) 
    [🔗](){id=sublime_Selection_subtract_}

    Subtract a region from the selection, such that the whole region is no longer contained within the selection.

9.  contains(_region: [Region][Region]_) → [bool][py3_bool] 
    [🔗](){id=sublime_Selection_contains_}
...
02. id() → [int][py3_int] 
    [🔗](){id=sublime_Sheet_id_}

    Returns:

    A number that uniquely identifies this sheet.

03. window() → [Window][Window] | [None][py3_None] 
    [🔗](){id=sublime_Sheet_window_}

    Returns:

    The [`Window`][Window] containing this sheet. May be `None` if the sheet has been closed.

04. view() → [View][View] | [None][py3_None] 
    [🔗](){id=sublime_Sheet_view_}
...
07. is_transient() → [bool][py3_bool]  4080
    ...
    Whether this sheet is transient.

08. is_selected() → [bool][py3_bool] 
    [🔗](){id=sublime_Sheet_is_selected_}
...
1.  Bases: [`Sheet`][Sheet]
    ...
    Specialization for sheets containing editable text, ie. a [`View`][View].

2.  set_name(_name: [str][py3_str]_) 
    [🔗](){id=sublime_TextSheet_set_name_}
...
2.  set_name(_name: [str][py3_str]_) 
    [🔗](){id=sublime_HtmlSheet_set_name_}

    Set the name displayed in the tab.

3.  set_contents(_contents: [str][py3_str]_) 
    [🔗](){id=sublime_HtmlSheet_set_contents_}
...
1.  Bases: [`object`][py3_object]
    ...
    Represents a single stack frame in the syntax highlighting. See [`View.context_backtrace`][context_backtrace].

2.  context_name _: [str][py3_str]_ 
    [🔗](){id=sublime_ContextStackFrame_context_name_}

    The name of the context.

3.  source_file _: [str][py3_str]_ 
    [🔗](){id=sublime_ContextStackFrame_source_file_}

    The name of the file the context is defined in.

4.  source_location _: [tuple][py3_tuple]\[[int][py3_int], [int][py3_int]\]_ 
    [🔗](){id=sublime_ContextStackFrame_source_location_}
...
[🔗](){id=sublime_View_}
_______________________________________________________________________________
## _class_ sublime.View 
_______________________________________________________________________________

01. Bases: [`object`][py3_object]

    Represents a view into a text [`Buffer`][Buffer].

    Note that multiple views may refer to the same [`Buffer`][Buffer], but they have their own unique selection and geometry. A list of these may be gotten using [`View.clones()`][clones] or [`Buffer.views()`][views].
...
03. buffer_id() → [int][py3_int] 
    [🔗](){id=sublime_View_buffer_id_}

    Returns:

    A number that uniquely identifies this view’s [`Buffer`][Buffer].

04. buffer() → [Buffer][Buffer] 
    [🔗](){id=sublime_View_buffer_}
...
07. element() → [str][py3_str] | [None][py3_None]  4050
    ...
    `None` for normal views that are part of a [`Sheet`][Sheet]. For views that comprise part of the UI a string is returned from the following list:
...
08. is_valid() → [bool][py3_bool] 
    [🔗](){id=sublime_View_is_valid_}

    Check whether this view is still valid. Will return `False` for a closed view, for example.

09. is_primary() → [bool][py3_bool] 
    [🔗](){id=sublime_View_is_primary_}

    Returns:

    Whether view is the primary view into a [`Buffer`][Buffer]. Will only be `False` if the user has opened multiple views into a file.

10. window() → [Window][Window] | [None][py3_None] 
    [🔗](){id=sublime_View_window_}

    Returns:

    A reference to the window containing the view, if any.

11. clones() → [list][py3_list]\[[sublime.View][View]\] 
    [🔗](){id=sublime_View_clones_}

    Returns:

    All the other views into the same [`Buffer`][Buffer]. See [`View`][View].

12. file_name() → [str][py3_str] | [None][py3_None] 
    [🔗](){id=sublime_View_file_name_}

    Returns:

    The full name of the file associated with the sheet, or `None` if it doesn’t exist on disk.

13. close(_on_close=<function View.<lambda>>_) → [bool][py3_bool] 
    [🔗](){id=sublime_View_close_}

    Closes the view.

14. retarget(_new_fname: [str][py3_str]_) 
    [🔗](){id=sublime_View_retarget_}

    Change the file path the buffer will save to.

15. name() → [str][py3_str] 
    [🔗](){id=sublime_View_name_}

    Returns:

    The name assigned to the buffer, if any.

16. set_name(_name: [str][py3_str]_) 
    [🔗](){id=sublime_View_set_name_}

    Assign a name to the buffer. Displayed as in the tab for unsaved files.

17. reset_reference_document() 
    [🔗](){id=sublime_View_reset_reference_document_}

    Clears the state of the [incremental diff][incremental_diff] for the view.

18. set_reference_document(_reference: [str][py3_str]_) 
    [🔗](){id=sublime_View_set_reference_document_}

    Uses the string reference to calculate the initial diff for the [incremental diff][incremental_diff].

19. is_loading() → [bool][py3_bool] 
    [🔗](){id=sublime_View_is_loading_}

    Returns:

    Whether the buffer is still loading from disk, and not ready for use.

20. is_dirty() → [bool][py3_bool] 
    [🔗](){id=sublime_View_is_dirty_}

    Returns:

    Whether there are any unsaved modifications to the buffer.

21. is_read_only() → [bool][py3_bool] 
    [🔗](){id=sublime_View_is_read_only_}

    Returns:

    Whether the buffer may not be modified.

22. set_read_only(_read_only: [bool][py3_bool]_) 
    [🔗](){id=sublime_View_set_read_only_}

    Set the read only property on the buffer.

23. is_scratch() → [bool][py3_bool] 
    [🔗](){id=sublime_View_is_scratch_}

    Returns:

    Whether the buffer is a scratch buffer. See [`set_scratch()`][set_scratch].

24. set_scratch(_scratch: [bool][py3_bool]_) 
    [🔗](){id=sublime_View_set_scratch_}
...
25. encoding() → [str][py3_str] 
    [🔗](){id=sublime_View_encoding_}

    Returns:

    The encoding currently associated with the buffer.

26. set_encoding(_encoding_name: [str][py3_str]_) 
    [🔗](){id=sublime_View_set_encoding_}

    Applies a new encoding to the file. This will be used when the file is saved.

27. line_endings() → [str][py3_str] 
    [🔗](){id=sublime_View_line_endings_}

    Returns:

    The encoding currently associated with the file.

28. set_line_endings(_line_ending_name: [str][py3_str]_) 
    [🔗](){id=sublime_View_set_line_endings_}

    Sets the line endings that will be applied when next saving.

29. size() → [int][py3_int] 
    [🔗](){id=sublime_View_size_}

    Returns:

    The number of character in the file.

30. insert(_edit: [Edit][Edit]_, _pt: [Point][Point]_, _text: [str][py3_str]_) → [int][py3_int] 
    [🔗](){id=sublime_View_insert_}
    ...
    An [`Edit`][Edit] object provided by a [`TextCommand`][TextCommand].
    ...
    If the [`Edit`][Edit] object is in an invalid state, ie. outside of a [`TextCommand`][TextCommand].

31. erase(_edit: [Edit][Edit]_, _region: [Region][Region]_) 
    [🔗](){id=sublime_View_erase_}

    Erases the contents of the provided [`Region`][Region] from the buffer.

32. replace(_edit: [Edit][Edit]_, _region: [Region][Region]_, _text: [str][py3_str]_) 
    [🔗](){id=sublime_View_replace_}

    Replaces the contents of the [`Region`][Region] in the buffer with the provided string.

33. change_count() → [int][py3_int] 
    [🔗](){id=sublime_View_change_count_}
...
34. change_id() → [tuple][py3_tuple]\[[int][py3_int], [int][py3_int], [int][py3_int]\] 
    [🔗](){id=sublime_View_change_id_}

    Get a 3-element tuple that can be passed to [`transform_region_from()`][transform_region_from] to obtain a region equivalent to a region of the view in the past. This is primarily useful for plugins providing text modification that must operate in an asynchronous fashion and must be able to handle the view contents changing between the request and response.

35. transform_region_from(_region: [Region][Region]_, _change_id: [tuple][py3_tuple]\[[int][py3_int], [int][py3_int], [int][py3_int]\]_) → [Region][Region] 
    [🔗](){id=sublime_View_transform_region_from_}

    Transforms a region from a previous point in time to an equivalent region in the current state of the View. The `change_id` must have been obtained from [`change_id()`][change_id] at the point in time the region is from.

36. run_command(_cmd: [str][py3_str]_, _args: [CommandArgs][CommandArgs] = None_) 
    [🔗](){id=sublime_View_run_command_}

    Run the named [`TextCommand`][TextCommand] with the (optional) given `args`.

37. sel() → [Selection][Selection] 
    [🔗](){id=sublime_View_sel_}

    Returns:

    The views [`Selection`][Selection].

38. substr(_x: [Region][Region] | [Point][Point]_) → [str][py3_str] 
    [🔗](){id=sublime_View_substr_}

    Returns:

    The string at the [`Point`][Point] or within the [`Region`][Region] provided.

39. find(_pattern: [str][py3_str]_, _start_pt: [Point][Point]_, _flags=FindFlags.NONE_) → [Region][Region] 
    [🔗](){id=sublime_View_find_}
    ...
    The [`Point`][Point] to start searching from.

    `flags`

    Controls various behaviors of find. See [`FindFlags`][FindFlags].

    Returns:

    The first [`Region`][Region] matching the provided pattern.

40. find_all(_pattern: [str][py3_str]_, _flags=FindFlags.NONE_, _fmt: [str][py3_str] | [None][py3_None] = None_, _extractions: [list][py3_list]\[[str][py3_str]\] | [None][py3_None] = None_) → [list][py3_list]\[[sublime.Region][Region]\] 
    [🔗](){id=sublime_View_find_all_}
    ...
    To reference match group, use variable like this "$0" (the whole pattern math), "$1", "$2", "$3" and so.
...
41. settings() → [Settings][Settings] 
    [🔗](){id=sublime_View_settings_}

    Returns:

    The view’s [`Settings`][Settings] object. Any changes to it will be private to this view.

42. meta_info(_key: [str][py3_str]_, _pt: [Point][Point]_) → [Value][Value] 
    [🔗](){id=sublime_View_meta_info_}

    Look up the preference `key` for the scope at the provided [`Point`][Point] from all matching `.tmPreferences` files.

    Examples of keys are `TM_COMMENT_START` and `showInSymbolList`.

43. extract_tokens_with_scopes(_region: [Region][Region]_) → [list][py3_list]\[[tuple][py3_tuple]\[[sublime.Region][Region], [str][py3_str]\]\] 
    [🔗](){id=sublime_View_extract_tokens_with_scopes_}
    ...
    A list of pairs containing the [`Region`][Region] and the scope of each token.

44. extract_scope(_pt: [Point][Point]_) → [Region][Region] 
    [🔗](){id=sublime_View_extract_scope_}

    Returns:

    The extent of the syntax scope name assigned to the character at the given [`Point`][Point], narrower syntax scope names included.

45. expand_to_scope(_pt: [Point][Point]_, _selector: [str][py3_str]_) → [Region][Region] | [None][py3_None] 
    [🔗](){id=sublime_View_expand_to_scope_}

    Expand by the provided scope selector from the [`Point`][Point].
    ...
    The matched [`Region`][Region], if any.

46. scope_name(_pt: [Point][Point]_) → [str][py3_str] 
    [🔗](){id=sublime_View_scope_name_}
...
48. match_selector(_pt: [Point][Point]_, _selector: [str][py3_str]_) → [bool][py3_bool] 
    [🔗](){id=sublime_View_match_selector_}

    Returns:

    Whether the provided scope selector matches the [`Point`][Point].

49. score_selector(_pt: [Point][Point]_, _selector: [str][py3_str]_) → [int][py3_int] 
    [🔗](){id=sublime_View_score_selector_}

    Equivalent to:

    sublime.score_selector(view.scope_name(pt), selector)

    See [`sublime.score_selector`][score_selector].

50. find_by_selector(_selector: [str][py3_str]_) → [list][py3_list]\[[sublime.Region][Region]\] 
    [🔗](){id=sublime_View_find_by_selector_}
...
52. style_for_scope(_scope: [str][py3_str]_) → [dict][py3_dict]\[[str][py3_str], [str][py3_str]\] 
    [🔗](){id=sublime_View_style_for_scope_}
...
53. lines(_region: [Region][Region]_) → [list][py3_list]\[[sublime.Region][Region]\] 
    [🔗](){id=sublime_View_lines_}

    Returns:

    A list of lines (in sorted order) intersecting the provided [`Region`][Region].

54. split_by_newlines(_region: [Region][Region]_) → [list][py3_list]\[[sublime.Region][Region]\] 
    [🔗](){id=sublime_View_split_by_newlines_}

    Splits the region up such that each [`Region`][Region] returned exists on exactly one line.

55. line(_x: [Region][Region] | [Point][Point]_) → [Region][Region] 
    [🔗](){id=sublime_View_line_}

    Returns:

    The line that contains the [`Point`][Point] or an expanded [`Region`][Region] to the beginning/end of lines, excluding the newline character.

56. full_line(_x: [Region][Region] | [Point][Point]_) → [Region][Region] 
    [🔗](){id=sublime_View_full_line_}

    Returns:

    The line that contains the [`Point`][Point] or an expanded [`Region`][Region] to the beginning/end of lines, including the newline character.

57. word(_x: [Region][Region] | [Point][Point]_) → [Region][Region] 
    [🔗](){id=sublime_View_word_}

    Returns:

    The word that contains the provided [`Point`][Point]. If a [`Region`][Region] is provided it’s beginning/end are expanded to word boundaries.

58. classify(_pt: [Point][Point]_) → [PointClassification][PointClassification] 
    [🔗](){id=sublime_View_classify_}

    Classify the provided[](#sublime.Point "</x-turndown")"sublime.Point">`Point`. See [`PointClassification`][PointClassification].

59. find_by_class(_pt: [Point][Point]_, _forward: [bool][py3_bool]_, _classes: [PointClassification][PointClassification]_, _separators=''_, _sub_word_separators=''_) → [Point][Point] 
    [🔗](){id=sublime_View_find_by_class_}

    Find the next location that matches the provided [`PointClassification`][PointClassification].
...
60. expand_by_class(_x: [Region][Region] | [Point][Point]_, _classes: [PointClassification][PointClassification]_, _separators=''_, _sub_word_separators=''_) → [Region][Region] 
    [🔗](){id=sublime_View_expand_by_class_}

    Expand the provided [`Point`][Point] or [`Region`][Region] to the left and right until each side lands on a location that matches the provided [`PointClassification`][PointClassification]. See [`find_by_class`][find_by_class].
...
61. rowcol(_tp: [Point][Point]_) → [tuple][py3_tuple]\[[int][py3_int], [int][py3_int]\] 
    [🔗](){id=sublime_View_rowcol_}
...
64. text_point(_row: [int][py3_int]_, _col: [int][py3_int]_, _\*_, _clamp_column=False_) → [Point][Point] 
    [🔗](){id=sublime_View_text_point_}
...
65. text_point_utf8(_row: [int][py3_int]_, _col: [int][py3_int]_, _\*_, _clamp_column=False_) → [Point][Point] 
    [🔗](){id=sublime_View_text_point_utf8_}
...
66. text_point_utf16(_row: [int][py3_int]_, _col: [int][py3_int]_, _\*_, _clamp_column=False_) → [Point][Point] 
    [🔗](){id=sublime_View_text_point_utf16_}
...
67. visible_region() → [Region][Region] 
    [🔗](){id=sublime_View_visible_region_}

    Returns:

    The currently visible area of the view.

68. show(_location: [Region][Region] | [Selection][Selection] | [Point][Point]_, _show_surrounds=True_, _keep_to_left=False_, _animate=True_) 
    [🔗](){id=sublime_View_show_}
    ...
    The location to scroll the view to. For a [`Selection`][Selection] only the first [`Region`][Region] is shown.
...
69. show_at_center(_location: [Region][Region] | [Point][Point]_, _animate=True_) 
    [🔗](){id=sublime_View_show_at_center_}
...
70. viewport_position() → [Vector][Vector] 
    [🔗](){id=sublime_View_viewport_position_}

    Returns:

    The offset of the viewport in layout coordinates.

71. set_viewport_position(_xy: [Vector][Vector]_, _animate=True_) 
    [🔗](){id=sublime_View_set_viewport_position_}

    Scrolls the viewport to the given layout position.

72. viewport_extent() → [Vector][Vector] 
    [🔗](){id=sublime_View_viewport_extent_}

    Returns:

    The width and height of the viewport.

73. layout_extent() → [Vector][Vector] 
    [🔗](){id=sublime_View_layout_extent_}

    Returns:

    The width and height of the layout.

74. text_to_layout(_tp: [Point][Point]_) → [Vector][Vector] 
    [🔗](){id=sublime_View_text_to_layout_}

    Convert a text point to a layout position.

75. text_to_window(_tp: [Point][Point]_) → [Vector][Vector] 
    [🔗](){id=sublime_View_text_to_window_}

    Convert a text point to a window position.

76. layout_to_text(_xy: [Vector][Vector]_) → [Point][Point] 
    [🔗](){id=sublime_View_layout_to_text_}

    Convert a layout position to a text point.

77. layout_to_window(_xy: [Vector][Vector]_) → [Vector][Vector] 
    [🔗](){id=sublime_View_layout_to_window_}

    Convert a layout position to a window position.

78. window_to_layout(_xy: [Vector][Vector]_) → [Vector][Vector] 
    [🔗](){id=sublime_View_window_to_layout_}

    Convert a window position to a layout position.

79. window_to_text(_xy: [Vector][Vector]_) → [Point][Point] 
    [🔗](){id=sublime_View_window_to_text_}

    Convert a window position to a text point.

80. line_height() → [DIP][DIP] 
    [🔗](){id=sublime_View_line_height_}
...
81. em_width() → [DIP][DIP] 
    [🔗](){id=sublime_View_em_width_}

    Returns:

    The typical character width used in the layout.

82. is_folded(_region: [Region][Region]_) → [bool][py3_bool] 
    [🔗](){id=sublime_View_is_folded_}

    Returns:

    Whether the provided [`Region`][Region] is folded.

83. folded_regions() → [list][py3_list]\[[sublime.Region][Region]\] 
    [🔗](){id=sublime_View_folded_regions_}

    Returns:

    The list of folded regions.

84. fold(_x: [Region][Region] | [list][py3_list]\[[sublime.Region][Region]\]_) → [bool][py3_bool] 
    [🔗](){id=sublime_View_fold_}

    Fold the provided [`Region`][Region] (s).

    Returns:

    `False` if the regions were already folded.

85. unfold(_x: [Region][Region] | [list][py3_list]\[[sublime.Region][Region]\]_) → [list][py3_list]\[[sublime.Region][Region]\] 
    [🔗](){id=sublime_View_unfold_}

    Unfold all text in the provided [`Region`][Region] (s).

    Returns:

    The unfolded regions.

86. add_regions(_key: [str][py3_str]_, _regions: [list][py3_list]\[[sublime.Region][Region]\]_, _scope=''_, _icon=''_, _flags=RegionFlags.NONE_, _annotations: [list][py3_list]\[[str][py3_str]\] = \[\]_, _annotation_color=''_, _on_navigate: [Callable][py3_typing_Callable]\[\[[str][py3_str]\], [None][py3_None]\] | [None][py3_None] = None_, _on_close: [Callable][py3_typing_Callable]\[\[\], [None][py3_None]\] | [None][py3_None] = None_) 
    [🔗](){id=sublime_View_add_regions_}
    ...
    An identifier for the collection of regions. If a set of regions already exists for this key they will be overridden. See [`get_regions`][get_regions].
...
87. get_regions(_key: [str][py3_str]_) → [list][py3_list]\[[sublime.Region][Region]\] 
    [🔗](){id=sublime_View_get_regions_}

    Returns:

    The regions associated with the given `key`, if any.

88. erase_regions(_key: [str][py3_str]_) 
    [🔗](){id=sublime_View_erase_regions_}

    Remove the regions associated with the given `key`.

89. assign_syntax(_syntax: [str][py3_str] | [Syntax][Syntax]_) 
    [🔗](){id=sublime_View_assign_syntax_}
...
90. set_syntax_file(_syntax_file: [str][py3_str]_) 
    ...
    Use [`assign_syntax()`][assign_syntax] instead.

91. syntax() → [Syntax][Syntax] | [None][py3_None] 
    [🔗](){id=sublime_View_syntax_}

    Returns:

    The syntax assigned to the buffer.

92. symbols() → [list][py3_list]\[[tuple][py3_tuple]\[[sublime.Region][Region], [str][py3_str]\]\] 
    [🔗](){id=sublime_View_symbols_}
...
93. get_symbols() → [list][py3_list]\[[tuple][py3_tuple]\[[sublime.Region][Region], [str][py3_str]\]\] 
    [🔗](){id=sublime_View_get_symbols_}
...
94. indexed_symbols() → [list][py3_list]\[[tuple][py3_tuple]\[[sublime.Region][Region], [str][py3_str]\]\]  3148
    ...
    A list of the [`Region`][Region] and name of symbols.
...
98. set_status(_key: [str][py3_str]_, _value: [str][py3_str]_) 
    [🔗](){id=sublime_View_set_status_}
...
99. get_status(_key: [str][py3_str]_) → [str][py3_str] 
    [🔗](){id=sublime_View_get_status_}

    Returns:

    The previous assigned value associated with the given `key`, if any.

    See [`set_status()`][set_status].

100. erase_status(_key: [str][py3_str]_) 
    [🔗](){id=sublime_View_erase_status_}
...
101. extract_completions(_prefix: [str][py3_str]_, _tp: [Point][Point] = \-1_) → [list][py3_list]\[[str][py3_str]\] 
    ...
    The [`Point`][Point] by which to weigh words. Closer words are preferred.

102. command_history(_index: [int][py3_int]_, _modifying_only=False_) → [tuple][py3_tuple]\[[str][py3_str], [CommandArgs][CommandArgs], [int][py3_int]\] 
    [🔗](){id=sublime_View_command_history_}
...
103. overwrite_status() → [bool][py3_bool] 
    [🔗](){id=sublime_View_overwrite_status_}

    Returns:

    The overwrite status, which the user normally toggles via the insert key.

104. set_overwrite_status(_value: [bool][py3_bool]_) 
    [🔗](){id=sublime_View_set_overwrite_status_}

    Set the overwrite status. See [`overwrite_status()`][overwrite_status].

105. show_popup_menu(_items: [list][py3_list]\[[str][py3_str]\]_, _on_done: [Callable][py3_typing_Callable]\[\[[int][py3_int]\], [None][py3_None]\]_, _flags=0_) 
    [🔗](){id=sublime_View_show_popup_menu_}
...
106. show_popup(_content: [str][py3_str]_, _flags=PopupFlags.NONE_, _location: [Point][Point] = \-1_, _max_width: [DIP][DIP] = 320_, _max_height: [DIP][DIP] = 240_, _on_navigate: Callable\[\[[str][py3_str]\], [None][py3_None]\] | [None][py3_None] = None_, _on_hide: Callable\[\[\], [None][py3_None]\] | [None][py3_None] = None_) 
    [🔗](){id=sublime_View_show_popup_}
    ...
    Flags controlling popup behavior. See [`PopupFlags`][PopupFlags].

    `location`

    The [`Point`][Point] at which to display the popup. If `-1` the popup is shown at the current postion of the caret.
...
107. update_popup(_content: [str][py3_str]_) 
    [🔗](){id=sublime_View_update_popup_}

    Update the content of the currently visible popup.

108. is_popup_visible() → [bool][py3_bool] 
    [🔗](){id=sublime_View_is_popup_visible_}
...
109. hide_popup() 
    [🔗](){id=sublime_View_hide_popup_}

    Hide the current popup.

110. is_auto_complete_visible() → [bool][py3_bool] 
    [🔗](){id=sublime_View_is_auto_complete_visible_}
...
112. export_to_html(_regions: [Region][Region] | [list][py3_list]\[[sublime.Region][Region]\] | [None][py3_None] = None_, _minihtml=False_, _enclosing_tags=False_, _font_size=True_, _font_family=True_)  4092
    ...
    Whether the exported HTML should be compatible with [minihtml Reference][minihtml].
...
4.  views() → [list][py3_list]\[[sublime.View][View]\] 
    [🔗](){id=sublime_Buffer_views_}

    Returns a list of all views that are associated with this buffer.

5.  primary_view() → [View][View] 
    [🔗](){id=sublime_Buffer_primary_view_}

    The primary view associated with this buffer.

[🔗](){id=sublime_Settings_}
_______________________________________________________________________________
## _class_ sublime.Settings 
_______________________________________________________________________________
...
08. has(_key: [str][py3_str]_) → [bool][py3_bool] 
    ...
    Same as [`__contains__`][__contains__].

09. set(_key: [str][py3_str]_, _value: [Value][Value]_) 
    [🔗](){id=sublime_Settings_set_}

    Same as [`__setitem__`][__setitem__].

10. erase(_key: [str][py3_str]_) 
    [🔗](){id=sublime_Settings_erase_}

    Same as [`__delitem__`][__delitem__].

11. add_on_change(_tag: [str][py3_str]_, _callback: [Callable][py3_typing_Callable]\[\[\], [None][py3_None]\]_) 
    [🔗](){id=sublime_Settings_add_on_change_}
    ...
    A string associated with the callback. For use with [`clear_on_change`][clear_on_change].

    `callback`

    A callable object to be run when a setting is changed.

12. clear_on_change(_tag: [str][py3_str]_) 
    [🔗](){id=sublime_Settings_clear_on_change_}

    Remove all callbacks associated with the provided `tag`. See [`add_on_change`][add_on_change].

[🔗](){id=sublime_Phantom_}
_______________________________________________________________________________
## _class_ sublime.Phantom 
_______________________________________________________________________________

1.  Bases: [`object`][py3_object]

    Represents an [minihtml Reference][minihtml]\-based decoration to display non-editable content interspersed in a [`View`][View]. Used with [`PhantomSet`][PhantomSet] to actually add the phantoms to the [`View`][View]. Once a [`Phantom`][Phantom] has been constructed and added to the [`View`][View], changes to the attributes will have no effect.

2.  region _: [Region][Region]_ 
    [🔗](){id=sublime_Phantom_region_}

    The [`Region`][Region] associated with the phantom. The phantom is displayed at the start of the [`Region`][Region].

3.  content _: [str][py3_str]_ 
    [🔗](){id=sublime_Phantom_content_}

    The HTML content of the phantom.

4.  layout _: [PhantomLayout][PhantomLayout]_ 
    [🔗](){id=sublime_Phantom_layout_}

    How the phantom should be placed relative to the `region`.

5.  on_navigate _: [Callable][py3_typing_Callable]\[\[[str][py3_str]\], [None][py3_None]\] | [None][py3_None]_ 
    [🔗](){id=sublime_Phantom_on_navigate_}

    Called when a link in the HTML is clicked. The value of the `href` attribute is passed.

6.  to_tuple() → [tuple][py3_tuple]\[[tuple][py3_tuple]\[[Point][Point], [Point][Point]\], [str][py3_str], [PhantomLayout][PhantomLayout], Callable\[\[[str][py3_str]\], [None][py3_None]\] | [None][py3_None]\] 
    [🔗](){id=sublime_Phantom_to_tuple_}
...
[🔗](){id=sublime_PhantomSet_}
_______________________________________________________________________________
## _class_ sublime.PhantomSet 
_______________________________________________________________________________

1.  Bases: [`object`][py3_object]

    A collection that manages [`Phantom`][Phantom] objects and the process of adding them, updating them and removing them from a [`View`][View].

2.  `__init__`(_view_, _key=''_) 
    [🔗](){id=sublime_PhantomSet___init___}

3.  view _: [View][View]_ 
    [🔗](){id=sublime_PhantomSet_view_}

    The [`View`][View] the phantom set is attached to.

4.  key: [str][py3_str] 
    [🔗](){id=sublime_PhantomSet_key_}

    A string used to group the phantoms together.

5.  update(_phantoms: [Iterable][py3_typing_Iterable]\[[Phantom][Phantom]\]_) 
    [🔗](){id=sublime_PhantomSet_update_}

    Update the set of phantoms. If the [`Phantom.region`][region] of existing phantoms have changed they will be moved; new phantoms are added and ones not present are removed.

[🔗](){id=sublime_Html_}
_______________________________________________________________________________
## _class_ sublime.Html 
_______________________________________________________________________________

1.  Bases: [`object`][py3_object]

    Used to indicate that a string is formatted as HTML. See [`CommandInputHandler.preview()`][preview].
...
2.  `__init__`(_completions: [list][py3_list]\[[CompletionValue][CompletionValue]\] | [None][py3_None] = None_, _flags=AutoCompleteFlags.NONE_) 
    [🔗](){id=sublime_CompletionList___init___}

    Parameters:

    `completions`

    If `None` is passed, the method [`set_completions()`][set_completions] must be called before the completions will be displayed to the user.

    `flags`

    Flags controlling auto-complete behavior. See [`AutoCompleteFlags`][AutoCompleteFlags].

3.  set_completions(_completions: [list][py3_list]\[[CompletionValue][CompletionValue]\]_, _flags=AutoCompleteFlags.NONE_) 
    [🔗](){id=sublime_CompletionList_set_completions_}
...
02. trigger _: [str][py3_str]_ 
    [🔗](){id=sublime_CompletionItem_trigger_}

    Text to match against the user’s input.

03. annotation _: [str][py3_str]_ 
    [🔗](){id=sublime_CompletionItem_annotation_}

    A hint to draw to the right-hand side of the trigger.

04. completion _: [str][py3_str]_ 
    [🔗](){id=sublime_CompletionItem_completion_}

    Text to insert if the completion is specified. If empty the [`trigger`][trigger] will be inserted instead.

05. completion_format _: [CompletionFormat][CompletionFormat]_ 
    [🔗](){id=sublime_CompletionItem_completion_format_}

    The format of the completion. See [`CompletionFormat`][CompletionFormat].

06. kind _: [Kind][Kind]_ 
    [🔗](){id=sublime_CompletionItem_kind_}
...
08. _classmethod_ snippet_completion(_trigger: [str][py3_str]_, _snippet: [str][py3_str]_, _annotation=''_, _kind=(KindId.SNIPPET, 's', 'Snippet')_, _details=''_) → [CompletionItem][CompletionItem] 
    [🔗](){id=sublime_CompletionItem_snippet_completion_}

    Specialized constructor for snippet completions. The [`completion_format`][completion_format] is always [`CompletionFormat.SNIPPET`][SNIPPET].

09. _classmethod_ command_completion(_trigger: [str][py3_str]_, _command: [str][py3_str]_, _args: [CommandArgs][CommandArgs] = None_, _annotation=''_, _kind=(KindId.AMBIGUOUS, '', '')_, _details=''_) → [CompletionItem][CompletionItem] 
    [🔗](){id=sublime_CompletionItem_command_completion_}

    Specialized constructor for command completions. The [`completion_format`][completion_format] is always [`CompletionFormat.COMMAND`][COMMAND].

10. sublime.list_syntaxes() → [list][py3_list]\[[sublime.Syntax][Syntax]\] 
    [🔗](){id=sublime_list_syntaxes_}

    list all known syntaxes.

    Returns a list of Syntax.

11. sublime.syntax_from_path(_path: [str][py3_str]_) → [Syntax][Syntax] | [None][py3_None] 
    [🔗](){id=sublime_syntax_from_path_}

    Get the syntax for a specific path.

    Returns a Syntax or None.

12. sublime.find_syntax_by_name(_name: [str][py3_str]_) → [list][py3_list]\[[sublime.Syntax][Syntax]\] 
    [🔗](){id=sublime_find_syntax_by_name_}

    Find syntaxes with the specified name.

    Name must match exactly. Return a list of Syntax.

13. sublime.find_syntax_by_scope(_scope: [str][py3_str]_) → [list][py3_list]\[[sublime.Syntax][Syntax]\] 
    [🔗](){id=sublime_find_syntax_by_scope_}

    Find syntaxes with the specified scope.

    Scope must match exactly. Return a list of Syntax.

14. sublime.find_syntax_for_file(_path_, _first_line=''_) → [Syntax][Syntax] | [None][py3_None] 
    [🔗](){id=sublime_find_syntax_for_file_}
...
2.  path _: [str][py3_str]_ 
    [🔗](){id=sublime_Syntax_path_}

    The packages path to the syntax file.

3.  name _: [str][py3_str]_ 
    [🔗](){id=sublime_Syntax_name_}

    The name of the syntax.

4.  hidden _: [bool][py3_bool]_ 
    [🔗](){id=sublime_Syntax_hidden_}

    If the syntax is hidden from the user.

5.  scope _: [str][py3_str]_ 
    [🔗](){id=sublime_Syntax_scope_}
...
1.  Bases: [`object`][py3_object]
    ...
    Represents a row in the quick panel, shown via [`Window.show_quick_panel()`][show_quick_panel].

2.  trigger _: [str][py3_str]_ 
    [🔗](){id=sublime_QuickPanelItem_trigger_}

    Text to match against user’s input.

3.  details _: [str][py3_str] | [list][py3_list]\[[str][py3_str]\] | [tuple][py3_tuple]\[[str][py3_str]\]_ 
    [🔗](){id=sublime_QuickPanelItem_details_}

    A [minihtml Reference][minihtml] string or list of strings displayed below the trigger.

4.  annotation _: [str][py3_str]_ 
    [🔗](){id=sublime_QuickPanelItem_annotation_}

    Hint to draw to the right-hand side of the row.

5.  kind _: [Kind][Kind]_ 
    [🔗](){id=sublime_QuickPanelItem_kind_}

    The kind of the item. See [`Kind`][Kind].
...
1.  Bases: [`object`][py3_object]
    ...
    Represents a row shown via [`ListInputHandler`][ListInputHandler].

2.  text _: [str][py3_str]_ 
    [🔗](){id=sublime_ListInputItem_text_}

    Text to match against the user’s input.

3.  value _: Any_ 
    [🔗](){id=sublime_ListInputItem_value_}

    A [`Value`][Value] passed to the command if the row is selected.

4.  details _: [str][py3_str] | [list][py3_list]\[[str][py3_str]\] | [tuple][py3_tuple]\[[str][py3_str]\]_ 
    [🔗](){id=sublime_ListInputItem_details_}

    A [minihtml Reference][minihtml] string or list of strings displayed below the trigger.

5.  annotation _: [str][py3_str]_ 
    [🔗](){id=sublime_ListInputItem_annotation_}

    Hint to draw to the right-hand side of the row.

6.  kind _: [Kind][Kind]_ 
    [🔗](){id=sublime_ListInputItem_kind_}
...
1.  Bases: [`object`][py3_object]
    ...
    Contains information about a [`Region`][Region] of a [`View`][View] that contains a symbol.

2.  name _: [str][py3_str]_ 
    [🔗](){id=sublime_SymbolRegion_name_}

    The name of the symbol.

3.  region _: [Region][Region]_ 
    [🔗](){id=sublime_SymbolRegion_region_}

    The location of the symbol within the [`View`][View].

4.  syntax _: [str][py3_str]_ 
    [🔗](){id=sublime_SymbolRegion_syntax_}

    The name of the syntax for the symbol.

5.  type _: [SymbolType][SymbolType]_ 
    [🔗](){id=sublime_SymbolRegion_type_}

    The type of the symbol. See [`SymbolType`][SymbolType].

6.  kind _: [Kind][Kind]_ 
    [🔗](){id=sublime_SymbolRegion_kind_}

    The kind of the symbol. See [`Kind`][Kind].
...
2.  path _: [str][py3_str]_ 
    [🔗](){id=sublime_SymbolLocation_path_}

    The filesystem path to the file containing the symbol.

3.  display_name _: [str][py3_str]_ 
    [🔗](){id=sublime_SymbolLocation_display_name_}

    The project-relative path to the file containing the symbol.

4.  row _: [int][py3_int]_ 
    [🔗](){id=sublime_SymbolLocation_row_}

    The row of the file the symbol is contained on.

5.  col _: [int][py3_int]_ 
    [🔗](){id=sublime_SymbolLocation_col_}

    The column of the row that the symbol is contained on.

6.  syntax _: [str][py3_str]_ 
    [🔗](){id=sublime_SymbolLocation_syntax_}

    The name of the syntax for the symbol.

7.  type _: [SymbolType][SymbolType]_ 
    [🔗](){id=sublime_SymbolLocation_type_}

    The type of the symbol. See [`SymbolType`][SymbolType].

8.  kind _: [Kind][Kind]_ 
    [🔗](){id=sublime_SymbolLocation_kind_}

    The kind of the symbol. See [`Kind`][Kind].

[🔗](){id=module-sublime_plugin}
_______________________________________________________________________________
# `sublime_plugin` Module 
_______________________________________________________________________________

[🔗](){id=sublime_plugin_CommandInputHandler_}
_______________________________________________________________________________
## _class_ sublime_plugin.CommandInputHandler 
_______________________________________________________________________________

1.  Bases: [`object`][py3_object]

2.  name() → [str][py3_str] 
    [🔗](){id=sublime_plugin_CommandInputHandler_name_}
...
3.  placeholder() → [str][py3_str] 
    [🔗](){id=sublime_plugin_CommandInputHandler_placeholder_}

    Placeholder text is shown in the text entry box before the user has entered anything. Empty by default.

4.  initial_text() → [str][py3_str] 
    [🔗](){id=sublime_plugin_CommandInputHandler_initial_text_}
...
6.  preview(_text: [str][py3_str]_) → [str][py3_str] | [Html][Html] 
    [🔗](){id=sublime_plugin_CommandInputHandler_preview_}

    Called whenever the user changes the text in the entry box. The returned value (either plain text or HTML) will be shown in the preview area of the _Command Palette_.

7.  validate(_text: [str][py3_str]_) → [bool][py3_bool] 
    [🔗](){id=sublime_plugin_CommandInputHandler_validate_}

    Called whenever the user presses enter in the text entry box. Return `False` to disallow the current value.

8.  cancel() 
    [🔗](){id=sublime_plugin_CommandInputHandler_cancel_}

    Called when the input handler is canceled, either by the user pressing backspace or escape.

9.  confirm(_text: [str][py3_str]_) 
    [🔗](){id=sublime_plugin_CommandInputHandler_confirm_}

    Called when the input is accepted, after the user has pressed enter and the text has been validated.

10.  next_input(_args_) → [CommandInputHandler][CommandInputHandler] | [None][py3_None] 
    [🔗](){id=sublime_plugin_CommandInputHandler_next_input_}
...
[🔗](){id=sublime_plugin_BackInputHandler_}
_______________________________________________________________________________
## _class_ sublime_plugin.BackInputHandler 
_______________________________________________________________________________

1.  Bases: [`CommandInputHandler`][CommandInputHandler]

[🔗](){id=sublime_plugin_TextInputHandler_}
_______________________________________________________________________________
## _class_ sublime_plugin.TextInputHandler 
_______________________________________________________________________________

1.  Bases: [`CommandInputHandler`][CommandInputHandler]

    TextInputHandlers can be used to accept textual input in the _Command Palette_. Return a subclass of this from [`Command.input()`][input].

    _For an input handler to be shown to the user, the command returning the input handler MUST be made available in the Command Palette by adding the command to a Default.sublime-commands file._

2.  description(_text: [str][py3_str]_) → [str][py3_str] 
    [🔗](){id=sublime_plugin_TextInputHandler_description_}

    The text to show in the _Command Palette_ when this input handler is not at the top of the input handler stack. Defaults to the text the user entered.

[🔗](){id=sublime_plugin_ListInputHandler_}
_______________________________________________________________________________
## _class_ sublime_plugin.ListInputHandler 
_______________________________________________________________________________

1.  Bases: [`CommandInputHandler`][CommandInputHandler]

    ListInputHandlers can be used to accept a choice input from a list items in the _Command Palette_. Return a subclass of this from [`Command.input()`][input].

    _For an input handler to be shown to the user, the command returning the input handler MUST be made available in the Command Palette by adding the command to a Default.sublime-commands file._

2.  list_items() → [list][py3_list]\[[str][py3_str]\] | [tuple][py3_tuple]\[[list][py3_list]\[[str][py3_str]\], [int][py3_int]\] | [list][py3_list]\[[tuple][py3_tuple]\[[str][py3_str], [Value][Value]\]\] | [tuple][py3_tuple]\[[list][py3_list]\[[tuple][py3_tuple]\[[str][py3_str], [Value][Value]\]\], [int][py3_int]\] | [list][py3_list]\[[sublime.ListInputItem][ListInputItem]\] | [tuple][py3_tuple]\[[list][py3_list]\[[sublime.ListInputItem][ListInputItem]\], [int][py3_int]\] 
    [🔗](){id=sublime_plugin_ListInputHandler_list_items_}
...
3.  description(_value_, _text: [str][py3_str]_) → [str][py3_str] 
    ...
    The text to show in the _Command Palette_ when this input handler is not at the top of the input handler stack. Defaults to the text of the list item the user selected.

[🔗](){id=sublime_plugin_Command_}
_______________________________________________________________________________
## _class_ sublime_plugin.Command 
_______________________________________________________________________________

1.  Bases: [`object`][py3_object]

2.  name() → [str][py3_str] 
    [🔗](){id=sublime_plugin_Command_name_}

    Return the name of the command. By default this is derived from the name of the class.

3.  is_enabled() → [bool][py3_bool] 
    [🔗](){id=sublime_plugin_Command_is_enabled_}
...
4.  is_visible() → [bool][py3_bool] 
    [🔗](){id=sublime_plugin_Command_is_visible_}
...
5.  is_checked() → [bool][py3_bool] 
    [🔗](){id=sublime_plugin_Command_is_checked_}
...
6.  description() → [str][py3_str] | [None][py3_None] 
    [🔗](){id=sublime_plugin_Command_description_}
...
7.  want_event() → [bool][py3_bool] 
    [🔗](){id=sublime_plugin_Command_want_event_}
...
10.  run() 
    [🔗](){id=sublime_plugin_Command_run_}

    Called when the command is run. Command arguments are passed as keyword arguments.

[🔗](){id=sublime_plugin_ApplicationCommand_}
_______________________________________________________________________________
## _class_ sublime_plugin.ApplicationCommand 
_______________________________________________________________________________

1.  Bases: [`Command`][Command]

    A [`Command`][Command] instantiated just once.

[🔗](){id=sublime_plugin_WindowCommand_}
_______________________________________________________________________________
## _class_ sublime_plugin.WindowCommand 
_______________________________________________________________________________

1.  Bases: [`Command`][Command]

    A [`Command`][Command] instantiated once per window. The [`Window`][Window] object may be retrieved via [`self.window`][window].

2.  window _: [Window][Window]_ 
    [🔗](){id=sublime_plugin_WindowCommand_window_}

    The [`Window`][Window] this command is attached to.

[🔗](){id=sublime_plugin_TextCommand_}
_______________________________________________________________________________
## _class_ sublime_plugin.TextCommand 
_______________________________________________________________________________

1.  Bases: [`Command`][Command]

    A [`Command`][Command] instantiated once per [`View`][View]. The [`View`][View] object may be retrieved via [`self.view`][view].

2.  view _: [View][View]_ 
    [🔗](){id=sublime_plugin_TextCommand_view_}

    The [`View`][View] this command is attached to.

3.  run(_edit: [Edit][Edit]_) 
    [🔗](){id=sublime_plugin_TextCommand_run_}

    Called when the command is run. Command arguments are passed as keyword arguments.

[🔗](){id=sublime_plugin_EventListener_}
_______________________________________________________________________________
## _class_ sublime_plugin.EventListener 
_______________________________________________________________________________
...
4.  on_new(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_new_}

    Called when a new file is created.

5.  on_new_async(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_new_async_}
...
8.  on_clone(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_clone_}

    Called when a view is cloned from an existing one.

9.  on_clone_async(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_clone_async_}

    Called when a view is cloned from an existing one. Runs in a separate thread, and does not block the application.

10.  on_load(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_load_}

    Called when the file is finished loading.

11.  on_load_async(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_load_async_}
...
19.  on_pre_close(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_pre_close_}

    Called when a view is about to be closed. The view will still be in the window at this point.

20.  on_close(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_close_}

    Called when a view is closed (note, there may still be other views into the same buffer).

21.  on_pre_save(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_pre_save_}

    Called just before a view is saved.

22.  on_pre_save_async(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_pre_save_async_}

    Called just before a view is saved. Runs in a separate thread, and does not block the application.

23.  on_post_save(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_post_save_}

    Called after a view has been saved.

24.  on_post_save_async(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_post_save_async_}

    Called after a view has been saved. Runs in a separate thread, and does not block the application.

25.  on_modified(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_modified_}

    Called after changes have been made to a view.

26.  on_modified_async(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_modified_async_}

    Called after changes have been made to a view. Runs in a separate thread, and does not block the application.

27.  on_selection_modified(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_selection_modified_}

    Called after the selection has been modified in a view.

28.  on_selection_modified_async(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_selection_modified_async_}

    Called after the selection has been modified in a view. Runs in a separate thread, and does not block the application.

29.  on_activated(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_activated_}

    Called when a view gains input focus.

30.  on_activated_async(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_activated_async_}

    Called when a view gains input focus. Runs in a separate thread, and does not block the application.

31.  on_deactivated(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_deactivated_}

    Called when a view loses input focus.

32.  on_deactivated_async(_view: [View][View]_) 
    [🔗](){id=sublime_plugin_EventListener_on_deactivated_async_}

    Called when a view loses input focus. Runs in a separate thread, and does not block the application.

33.  on_hover(_view: [View][View]_, _point: [Point][Point]_, _hover_zone: [HoverZone][HoverZone]_) 
    [🔗](){id=sublime_plugin_EventListener_on_hover_}
...
34.  on_query_context(_view: [View][View]_, _key: [str][py3_str]_, _operator: [QueryOperator][QueryOperator]_, _operand: [str][py3_str]_, _match_all: [bool][py3_bool]_) → [bool][py3_bool] | [None][py3_None] 
    [🔗](){id=sublime_plugin_EventListener_on_query_context_}
...
35.  on_query_completions(_view: [View][View]_, _prefix: [str][py3_str]_, _locations: List\[[Point][Point]\]_) → [None][py3_None] | List\[[CompletionValue][CompletionValue]\] | Tuple\[List\[[CompletionValue][CompletionValue]\], [AutoCompleteFlags][AutoCompleteFlags]\] | [CompletionList][CompletionList] 
    [🔗](){id=sublime_plugin_EventListener_on_query_completions_}
...
36.  on_text_command(_view: [View][View]_, _command_name: [str][py3_str]_, _args: [CommandArgs][CommandArgs]_) 
    [🔗](){id=sublime_plugin_EventListener_on_text_command_}
...
37.  on_window_command(_window: [Window][Window]_, _command_name: [str][py3_str]_, _args: [CommandArgs][CommandArgs]_) 
    [🔗](){id=sublime_plugin_EventListener_on_window_command_}
...
38.  on_post_text_command(_view: [View][View]_, _command_name: [str][py3_str]_, _args: [CommandArgs][CommandArgs]_) 
    [🔗](){id=sublime_plugin_EventListener_on_post_text_command_}

    Called after a text command has been executed.

39.  on_post_window_command(_window: [Window][Window]_, _command_name: [str][py3_str], _args: [CommandArgs][CommandArgs]_) 
    [🔗](){id=sublime_plugin_EventListener_on_post_window_command_}
...
50.  on_pre_close_project(_window: [Window][Window]_) 
    [🔗](){id=sublime_plugin_EventListener_on_pre_close_project_}

    Called right before a project is closed, passed the Window object.

[🔗](){id=sublime_plugin_ViewEventListener_}
_______________________________________________________________________________
## _class_ sublime_plugin.ViewEventListener 
_______________________________________________________________________________
...
17.  on_modified() 
    [🔗](){id=sublime_plugin_ViewEventListener_on_modified_}

    Called after changes have been made to the view.

18.  on_modified_async() 
    [🔗](){id=sublime_plugin_ViewEventListener_on_modified_async_}

    Same as [`on_modified`][on_modified] but runs in a separate thread, not blocking the application.

19.  on_selection_modified() 
    [🔗](){id=sublime_plugin_ViewEventListener_on_selection_modified_}

    Called after the selection has been modified in the view.

20.  on_selection_modified_async() 
    [🔗](){id=sublime_plugin_ViewEventListener_on_selection_modified_async_}

    Called after the selection has been modified in the view. Runs in a separate thread, and does not block the application.

21.  on_activated() 
    [🔗](){id=sublime_plugin_ViewEventListener_on_activated_}

    Called when a view gains input focus.

22.  on_activated_async() 
    [🔗](){id=sublime_plugin_ViewEventListener_on_activated_async_}

    Called when the view gains input focus. Runs in a separate thread, and does not block the application.

23.  on_deactivated() 
    [🔗](){id=sublime_plugin_ViewEventListener_on_deactivated_}

    Called when the view loses input focus.

24.  on_deactivated_async() 
    [🔗](){id=sublime_plugin_ViewEventListener_on_deactivated_async_}

    Called when the view loses input focus. Runs in a separate thread, and does not block the application.

25.  on_hover(_point: [Point][Point]_, _hover_zone: [HoverZone][HoverZone]_) 
    [🔗](){id=sublime_plugin_ViewEventListener_on_hover_}
...
26.  on_query_context(_key: [str][py3_str]_, _operator: [QueryOperator][QueryOperator]_, _operand: [str][py3_str]_, _match_all: [bool][py3_bool]_) → [bool][py3_bool] | [None][py3_None] 
    [🔗](){id=sublime_plugin_ViewEventListener_on_query_context_}
...
27.  on_query_completions(_prefix: [str][py3_str]_, _locations: List\[[Point][Point]\]_) → [None][py3_None] | List\[[CompletionValue][CompletionValue]\] | Tuple\[List\[[CompletionValue][CompletionValue]\], [AutoCompleteFlags][AutoCompleteFlags]\] | [CompletionList][CompletionList] 
    [🔗](){id=sublime_plugin_ViewEventListener_on_query_completions_}
...
29.  on_post_text_command(_command_name: [str][py3_str]_, _args: [CommandArgs][CommandArgs]_) 
    [🔗](){id=sublime_plugin_ViewEventListener_on_post_text_command_}

    Called after a text command has been executed.

30.  _classmethod_ is_applicable(_settings: [Settings][Settings]_) → [bool][py3_bool] 
    [🔗](){id=sublime_plugin_ViewEventListener_is_applicable_}

    Returns:

    Whether this listener should apply to a view with the given [`Settings`][Settings].

31.  _classmethod_ applies_to_primary_view_only() → [bool][py3_bool] 
    [🔗](){id=sublime_plugin_ViewEventListener_applies_to_primary_view_only_}
...
1.  Bases: [`object`][py3_object]
    ...
    A class that provides event handling about text changes made to a specific Buffer. Is separate from [`ViewEventListener`][ViewEventListener] since multiple views can share a single buffer.

2.  on_text_changed(_changes: List\[[TextChange][TextChange]\]_) 
    [🔗](){id=sublime_plugin_TextChangeListener_on_text_changed_}
    ...
    Same as [`on_text_changed`][on_text_changed] but runs in a separate thread, not blocking the application.

3.  on_revert() 
    [🔗](){id=sublime_plugin_TextChangeListener_on_revert_}

    Called when the buffer is reverted.

    A revert does not trigger text changes. If the contents of the buffer are required here use [`View.substr`][substr].

4.  on_revert_async() 
    [🔗](){id=sublime_plugin_TextChangeListener_on_revert_async_}

    Same as [`on_revert`][on_revert] but runs in a separate thread, not blocking the application.

5.  on_reload() 
    [🔗](){id=sublime_plugin_TextChangeListener_on_reload_}

    Called when the buffer is reloaded.

    A reload does not trigger text changes. If the contents of the buffer are required here use [`View.substr`][substr].

6.  on_reload_async() 
    [🔗](){id=sublime_plugin_TextChangeListener_on_reload_async_}

    Same as [`on_reload`][on_reload] but runs in a separate thread, not blocking the application.

7.  _classmethod_ is_applicable(_buffer: [Buffer][Buffer]_) 
    [🔗](){id=sublime_plugin_TextChangeListener_is_applicable_}
...
8.  detach() 
    [🔗](){id=sublime_plugin_TextChangeListener_detach_}
...
9.  attach(_buffer: [Buffer][Buffer]_) 
    [🔗](){id=sublime_plugin_TextChangeListener_attach_}
...
10.  is_attached() → [bool][py3_bool] 
    [🔗](){id=sublime_plugin_TextChangeListener_is_attached_}

    Returns:

    whether the listener is receiving events from a buffer. May not be called from `__init__`.

[Introducing our Git client **Sublime Merge**![](https://www.sublimetext.com/images/merge_callout_windows@2x.png)](https://www.sublimemerge.com) 
...
---------------------------------------------------
Sublime API Footlinks
===================================================
---------------------------------------------------

  [py3_FileNotFoundError]: https://docs.python.org/3/library/exceptions.html#FileNotFoundError
  [py3_None]: https://docs.python.org/3/library/constants.html#None
  [py3_ValueError]: https://docs.python.org/3/library/exceptions.html#ValueError
  [py3_bool]: https://docs.python.org/3/library/functions.html#bool
  [py3_bytes]: https://docs.python.org/3/library/stdtypes.html#bytes
  [py3_dict]: https://docs.python.org/3/library/stdtypes.html#dict
  [py3_enum_IntEnum]: https://docs.python.org/3/library/enum.html#enum.IntEnum
  [py3_enum_IntFlag]: https://docs.python.org/3/library/enum.html#enum.IntFlag
  [py3_float]: https://docs.python.org/3/library/functions.html#float
  [py3_int]: https://docs.python.org/3/library/functions.html#int
  [py3_list]: https://docs.python.org/3/library/stdtypes.html#list
  [py3_object]: https://docs.python.org/3/library/functions.html#object
  [py3_str]: https://docs.python.org/3/library/stdtypes.html#str
  [py3_term-ca#llback]: https://docs.python.org/3/glossary.html#term-callback
  [py3_tuple]: https://docs.python.org/3/library/stdtypes.html#tuple
  [py3_typing_Callable]: https://docs.python.org/3/library/typing.html#typing.Callable
  [py3_typing_Iterable]: https://docs.python.org/3/library/typing.html#typing.Iterable
  [py3_typing_Literal]: https://docs.python.org/3/library/typing.html#typing.Literal

  [minihtml]: https://www.sublimetext.com/docs/minihtml.html
  [incremental_diff]: https://www.sublimetext.com/docs/incremental_diff.html
  [scope_naming]: https://www.sublimetext.com/docs/scope_naming.html

  [ADD_TO_SELECTION]: #sublime_NewFileFlags_ADD_TO_SELECTION_
  [ApplicationCommand]: #sublime_plugin_ApplicationCommand_
  [AutoCompleteFlags]: #sublime_AutoCompleteFlags_
  [BackInputHandler]: #sublime_plugin_BackInputHandler_
  [Buffer]: #sublime_Buffer_
  [COMMAND]: #sublime_CompletionFormat_COMMAND_
  [CommandArgs]: #sublime_CommandArgs_
  [CommandInputHandler]: #sublime_plugin_CommandInputHandler_
  [Command]: #sublime_plugin_Command_
  [CompletionFormat]: #sublime_CompletionFormat_
  [CompletionItem]: #sublime_CompletionItem_
  [CompletionList]: #sublime_CompletionList_
  [CompletionValue]: #sublime_CompletionValue_
  [ContextStackFrame]: #sublime_ContextStackFrame_
  [DIP]: #sublime_DIP_
  [DialogResult]: #sublime_DialogResult_
  [Edit]: #sublime_Edit_
  [EventListener]: #sublime_plugin_EventListener_
  [Event]: #sublime_Event_
  [FindFlags]: #sublime_FindFlags_
  [HistoricPosition]: #sublime_HistoricPosition_
  [HoverZone]: #sublime_HoverZone_
  [HtmlSheet]: #sublime_HtmlSheet_
  [Html]: #sublime_Html_
  [ImageSheet]: #sublime_ImageSheet_
  [KindId]: #sublime_KindId_
  [Kind]: #sublime_Kind_
  [ListInputHandler]: #sublime_plugin_ListInputHandler_
  [ListInputItem]: #sublime_ListInputItem_
  [NewFileFlags]: #sublime_NewFileFlags_
  [PhantomLayout]: #sublime_PhantomLayout_
  [PhantomSet]: #sublime_PhantomSet_
  [Phantom]: #sublime_Phantom_
  [PointClassification]: #sublime_PointClassification_
  [Point]: #sublime_Point_
  [PopupFlags]: #sublime_PopupFlags_
  [QueryOperator]: #sublime_QueryOperator_
  [QuickPanelFlags]: #sublime_QuickPanelFlags_
  [QuickPanelItem]: #sublime_QuickPanelItem_
  [REPLACE_MRU]: #sublime_NewFileFlags_REPLACE_MRU_
  [RegionFlags]: #sublime_RegionFlags_
  [Region]: #sublime_Region_
  [SNIPPET]: #sublime_CompletionFormat_SNIPPET_
  [Selection]: #sublime_Selection_
  [Settings]: #sublime_Settings_
  [Sheet]: #sublime_Sheet_
  [SymbolLocation]: #sublime_SymbolLocation_
  [SymbolRegion]: #sublime_SymbolRegion_
  [SymbolType]: #sublime_SymbolType_
  [Syntax]: #sublime_Syntax_
  [TRANSIENT]: #sublime_NewFileFlags_TRANSIENT_
  [TextChangeListener]: #sublime_plugin_TextChangeListener_
  [TextChange]: #sublime_TextChange_
  [TextCommand]: #sublime_plugin_TextCommand_
  [TextInputHandler]: #sublime_plugin_TextInputHandler_
  [TextSheet]: #sublime_TextSheet_
  [Value]: #sublime_Value_
  [Vector]: #sublime_Vector_
  [ViewEventListener]: #sublime_plugin_ViewEventListener_
  [View]: #sublime_View_
  [WANT_EVENT]: #sublime_QuickPanelFlags_WANT_EVENT_
  [WindowCommand]: #sublime_plugin_WindowCommand_
  [Window]: #sublime_Window_
  [__contains__]: #sublime_Region___contains___
  [__contains__]: #sublime_Settings___contains___
  [__delitem__]: #sublime_Settings___delitem___
  [__len__]: #sublime_Region___len___
  [__setitem__]: #sublime_Settings___setitem___
  [add_on_change]: #sublime_Settings_add_on_change_
  [add_regions]: #sublime_View_add_regions_
  [arch]: #sublime_arch_
  [assign_syntax]: #sublime_View_assign_syntax_
  [cache_path]: #sublime_cache_path_
  [change_id]: #sublime_View_change_id_
  [channel]: #sublime_channel_
  [classify]: #sublime_View_classify_
  [clear_on_change]: #sublime_Settings_clear_on_change_
  [clones]: #sublime_View_clones_
  [command_completion]: #sublime_CompletionItem_command_completion_
  [completion_format]: #sublime_CompletionItem_completion_format_
  [confirm]: #sublime_plugin_CommandInputHandler_confirm_
  [context_backtrace]: #sublime_View_context_backtrace_
  [create_output_panel]: #sublime_Window_create_output_panel_
  [executable_path]: #sublime_executable_path_
  [expand_variables]: #sublime_expand_variables_
  [find]: #sublime_View_find_
  [find_all]: #sublime_View_find_all_
  [find_by_class]: #sublime_View_find_by_class_
  [format_command]: #sublime_format_command_
  [get_clipboard_async]: #sublime_get_clipboard_async_
  [get_regions]: #sublime_View_get_regions_
  [indexed_symbol_regions]: #sublime_View_indexed_symbol_regions_
  [input]: #sublime_plugin_Command_input_
  [installed_packages_path]: #sublime_installed_packages_path_
  [layout]: #sublime_Window_layout_
  [new_file]: #sublime_Window_new_file_
  [new_html_sheet]: #sublime_Window_new_html_sheet_
  [on_hover]: #sublime_plugin_EventListener_on_hover_
  [on_hover]: #sublime_plugin_ViewEventListener_on_hover_
  [on_load]: #sublime_plugin_ViewEventListener_on_load_
  [on_modified]: #sublime_plugin_ViewEventListener_on_modified_
  [on_post_move]: #sublime_plugin_ViewEventListener_on_post_move_
  [on_post_save]: #sublime_plugin_ViewEventListener_on_post_save_
  [on_pre_save]: #sublime_plugin_ViewEventListener_on_pre_save_
  [on_query_context]: #sublime_plugin_EventListener_on_query_context_
  [on_query_context]: #sublime_plugin_ViewEventListener_on_query_context_
  [on_reload]: #sublime_plugin_TextChangeListener_on_reload_
  [on_reload]: #sublime_plugin_ViewEventListener_on_reload_
  [on_revert]: #sublime_plugin_TextChangeListener_on_revert_
  [on_revert]: #sublime_plugin_ViewEventListener_on_revert_
  [on_text_changed]: #sublime_plugin_TextChangeListener_on_text_changed_
  [open_file]: #sublime_Window_open_file_
  [overwrite_status]: #sublime_View_overwrite_status_
  [packages_path]: #sublime_packages_path_
  [platform]: #sublime_platform_
  [preview]: #sublime_plugin_CommandInputHandler_preview_
  [region]: #sublime_Phantom_region_
  [score_selector]: #sublime_score_selector_
  [select_folder_dialog]: #sublime_select_folder_dialog_
  [set_completions]: #sublime_CompletionList_set_completions_
  [set_scratch]: #sublime_View_set_scratch_
  [set_status]: #sublime_View_set_status_
  [show_popup]: #sublime_View_show_popup_
  [show_quick_panel]: #sublime_Window_show_quick_panel_
  [snippet_completion]: #sublime_CompletionItem_snippet_completion_
  [style_for_scope]: #sublime_View_style_for_scope_
  [substr]: #sublime_View_substr_
  [symbol_locations]: #sublime_Window_symbol_locations_
  [symbol_regions]: #sublime_View_symbol_regions_
  [transform_region_from]: #sublime_View_transform_region_from_
  [trigger]: #sublime_CompletionItem_trigger_
  [validate]: #sublime_plugin_CommandInputHandler_validate_
  [version]: #sublime_version_
  [view]: #sublime_plugin_TextCommand_view_
  [views]: #sublime_Buffer_views_
  [want_event]: #sublime_plugin_CommandInputHandler_want_event_
  [window]: #sublime_plugin_WindowCommand_window_
...
The `match` key is a regex, supporting features from the [Oniguruma regex engine](https://raw.githubusercontent.com/kkos/oniguruma/v6.9.1/doc/RE). In the above example, `\b` is used to ensure only word boundaries are matched, to ensure that words such as `elsewhere` are not considered keywords.
...
A list of strings, defining file extensions this syntax should be used for. _Extensions listed here will be shown in file dialog dropdowns on some operating systems._

If a file does not have a basename, e.g. .gitignore, the entirety of the filename including the leading `.` should be specified.

hidden\_file\_extensions
...
contexts:
  ...
  string:
    \- meta\_scope: string.quoted.double.c
    \- match: \\\\.
...
While editing in Sublime Text, you can check what scopes have been applied to the text under the caret by pressing **Ctrl**_+_**Shift**_+_**P** (Mac) or **Ctrl**_+_**Alt**_+_**Shift**_+_**P** (Windows/Linux).
...
### Meta Patterns[🔗](){id=doc_meta_patterns}
...
### Match Patterns[🔗](){id=doc_match_patterns}

A _match_ pattern can include the following keys:
...
\- match: (\`\`\`)(js|javascript)
  ...
  embed\_scope: meta.embedded.js.markdown
  escape: ^\`\`\`
  escape\_captures:
...
paragraph:
  \- match: '(\`\`\`)(py|python)'
    ...
    embed\_scope: source.python.embedded
    escape: ^\`\`\`
    escape\_captures:
...
#### Including an External Prototype[🔗](){id=doc_including_an_external_prototype}
...
string:
  \- meta\_include\_prototype: false
  ...
...
\---
name: C++
file\_extensions: \[cc, cpp\]
scope: source.c++
extends: Packages/C++/C.sublime-syntax

A syntax using inheritance will inherit the `variables` and `contexts` values from its parent syntax. All other top-level keys, such as `file_extensions` and `scope` will _not_ be inherited.

### Variable Inheritance[🔗](){id=doc_variable_inheritance}
...
### Context Inheritance[🔗](){id=doc_context_inheritance}
...
for\_stmt:
  \- match: \\(
    set: for\_stmt\_expr1
for\_stmt\_expr1:
  \- match: ';'
    set: for\_stmt\_expr2
  \- match: \\)
    pop: truep-Indicator">- include: expr for\_stmt\_expr2: \- match: ';' set: for\_stmt\_expr3 \- match: \\) pop: true \- include: expr for\_stmt\_expr3: \- match: \\) pop: true \- match: ';' scope: invalid.illegal.stray-semi-colon \- include: expr

### Advanced Stack Usage[🔗](){id=doc_advanced_stack_usage}

In C, symbols are often defined with the `typedef` keyword. So that _Goto Definition_ can pick these up, the symbols should have the `entity.name.type` scope attached to them.

Doing this can be a little tricky, as while typedefs are sometimes simple, they can get quite complex:

typedef int coordinate\_t;
...
main:
  \- match: \\btypedef\\b
    scope: keyword.control.c
    set: \[typedef\_after\_typename, typename\]
...
### PHP Heredocs[🔗](){id=doc_php_heredocs}
...
contexts:
  ...
  heredoc:
    \- meta\_scope: string.unquoted.heredoc
    \- match: ^\\1;
        pop: true
...
*   Ensure the file name starts with syntax\_test\_.
    ...
    contexts:
      paragraph:
        \- match: \\(
          ...
          embed\_scope: source.other.embedded
          escape: \\)
          escape\_captures:
...
*   **Match Pattern with `set` and `meta_content_scope`**
    ...
    contexts:
      function:
        \- meta\_content\_scope: meta.function
        \- match: '\[a-zA-Z0-9\_\]+'
      ...
      function-params:
        \- meta\_scope: meta.function.params
        \- match: \\)
...
*   **Embed Escape Match and Meta Scopes**
    ...
    contexts:
      context1:
        \- meta\_scope: meta.group
        \- meta\_content\_scope: meta.content
        \- match: \\'
...
*   **Multiple Target Push Actions with `clear_scopes`**
    ...
    contexts:
      main:
        \- meta\_content\_scope: meta.main
        \- match: '\[a-zA-Z0-9\]+\\b'
          scope: identifier
          push:
            \- context2
      ...
      context2:
        \- meta\_scope: meta.ctx2
        \- clear\_scopes: 1
    
      context3:
        \- meta\_scope: meta.ctx3
        \- clear\_scopes: 1
        \- match: \\n
          pop: true
...
_______________________________________________________________________________
## Syntax Definitions[🔗](#syntax-definitions "Permalink to this heading")
_______________________________________________________________________________
...
_______________________________________________________________________________
### Example Syntaxes[🔗](#example-syntaxes "Permalink to this heading")
_______________________________________________________________________________
...
_Avoid `entity.name.type.class` and `entity.name.type.struct` which unnecessarily nest scope labels under `type`._
...
The following scopes are punctuation scopes that are not embedded within other scopes. For instance, the [string.](#string) section includes documentation about scopes for string punctuation.
...
Programming languages use [source.](#source) as their base scope, whereas content uses `text.`. One of the biggest differences is the fact that many plugins and other dynamic functionality is disabled within `text.` scopes. [markup.](#markup) scopes are typically used within text.
...
_______________________________________________________________________________
### Minimal Scope Coverage[🔗](#minimal-scope-coverage "Permalink to this heading")
_______________________________________________________________________________
...
_______________________________________________________________________________
### `entity.name.` Colors[🔗](#entity-name-colors "Permalink to this heading")
_______________________________________________________________________________
...
### Predefined Classes[🔗](){id=doc_predefined_classes}
...
*   `margin-bottom`: positive [units](#units)
*   `margin-left`: positive [units](#units)
*   `position`: `static`, `relative`
*   `top`: positive and negative [units](#units)
*   `right`: positive and negative [units](#units)
*   `bottom`: positive and negative [units](#units)
*   `left`: positive and negative [units](#units)
*   `background-color`: [colors](#colors)
*   `font-family`: comma-separated list of font families
*   `font-size`: positive [units](#units)
...
*   `border-bottom-style`: [border-style](#border-style)
*   `border-left-style`: [border-style](#border-style)
*   `border-width`: positive [units](#units)
*   `border-top-width`: positive [units](#units)
*   `border-right-width`: positive [units](#units)
*   `border-bottom-width`: positive [units](#units)
*   `border-left-width`: positive [units](#units)
*   `border-color`: [colors](#colors)
*   `border-top-color`: [colors](#colors)
*   `border-right-color`: [colors](#colors)
*   `border-bottom-color`: [colors](#colors)
*   `border-left-color`: [colors](#colors)
*   `border-radius`: positive [units](#units)
*   `border-top-left-radius`: positive [units](#units)
...
#### `min-contrast()` Adjuster[🔗](){id=doc_min_contrast_adjuster}
...
#### Predefined Variables[🔗](){id=doc_predefined_variables}
...
*   [Restricted begin\_edit() and end\_edit()](#restricted-begin-edit-and-end-edit)
