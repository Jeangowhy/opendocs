...
事实上，输入输入文件是可以被重定向到其它文件的，如运行程序时，将 stdio 重定向到 out.txt 文件，而不是控制台默认的显示器输出：
...
在定义结构体或联合体时，实践中通常和 typedef 一起使用，这样方便定义结构体类型的变量，像以下这样，Treenode 就是一种结构体类型，而不像前面声明结构体紧接着声明结构体变量：
