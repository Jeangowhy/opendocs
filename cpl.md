...
* `|||🍀4.  Formatted input/output                                   ` [P61](#P61)
...
### ===👉 Array
  ...
  注意：在企业招聘考试中，会很鸡贼地考数组名的作用。比如 &im 代表什么？&im+1 又代表什么？其实数组名就是这个数组的变量名，它的地址就是指向这个数组的指针，所以 &im 就是指向一个二维数组的指针，&im+1 这样的指针运算就是指向偏移一整个二维数组大小的内存地址（指向另一个和 im 数组同样结构的数组）。
