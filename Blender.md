...
- Object 对象，属于 Blender 场景中的对象，包含这个对象基本信息：位置、旋转、缩放以及几何体数据；
- Geometry 几何体，由顶点、曲面（mesh）构建的对象；
- Instance 实例对象由几何体若其它节点（Instance on Points）生成的几何体副本；

这些对象可以使用以下节点节选进行转换：

- Object Info 可以获取对象的几何体对象；
- Geometry to Instance 几何体转实例；
- Instances to Points 实例转顶点，注意 Instance on Points 是在顶点上生成实例；
...
默认添加几何节点修改器的输入、输出分组的只有一个几何体连接端口。如果需要使用其它功能的对象，可以将
空端口（empty soket）连接到其它节点的端口上，Blender 会自动设置空端口的类型。可以打开侧栏面板（N）
设置这些端口的属性类型：Group -> Interface。

设置好端口，可以在几何节点修改器的属性面板中设置这些自定义端口的属性值，根据所设置的类型提供数据。

如果是节点组（Node Group）其输入输出端口设置类型，但是端口的值得直接在当前节点编辑器视图设置，
而不是在修改器面板上设置。使用 Ctrl+G 将框选的节点分组，点击分组节点右上角的图标编辑节点组。
进入节点组视图后，点击节点编辑器右上角的回钩状图标（Parent Node Tree）返回上一级节点编辑器视图。

比如，要设置明度，就需要设置一个 float 类型的输入端口，然后在修改器面板设置明度值。
...
有两个材质关系：笔刷设置的材质与蜡笔对象设置的材质。笔刷在蜡笔对象上作画的过程，就是记录
笔刷属性（包括笔刷材质）的过程，每当有新的笔刷作画时，蜡笔对象上设置的材质就会相应增加
材质插槽记录笔迹对应的材质，行迹数据中包含材质插槽序号，Blender 通过序号来获取材质数据，
进而渲染出笔迹。Blender 通过材质设置，为蜡笔对象的矢量笔迹增加了纹理贴图，贴图总是朝向镜头。
...
1. 曲面建模（NURBS Modeling），通过顶点变控制曲面的形成。
2. 参数化建模（Parametric Modeling），20 世纪末主要的计算机辅助设计方法。
3. 逆向建模（Reverse Modeling），基于实物外形，进行逆向处理得到建模。

蜡笔修改器（Grease Modifier）和其它曲面修改器一样，都是用于模型加工的一种工具（算法）。这里
特别指蜡笔修改器，是因为蜡笔对象建模是基于矢量曲线的建模技术，而曲面则是基于顶点集合的多边形建模
（Polygon Modeling）。因此两者修改器的算法实现上有很大的差别，互相之间也不通用，所以分类表示。

- 顶点数据修改类修改器（Modify Modifiers）
	- Texture Mapping Modifier 修改 stroke 纹理的 UV 坐标。
	- Time Offset Modifier 修改蜡笔关键帧的时间线偏移。
	- Vertex Weight Angle Modifier  基于确实角度修改顶点组的权重。
	- Vertex Weight Proximity Modifier 基于参考对象的距离场修改顶点组的权重。
- 曲线生成类修改器（Generate Modifiers）
	- Array 阵列修改器
	- Build 构建修改器
	- Mirror 镜像修改器
	- Simplify 简化修改器
	- Subdivide 细分修改器
- 曲线变形类修改器（Deform Modifiers）
	- Armature 骨架修改器
	- Hook 挂钩修改器
	- Lattice 晶格修改器
	- Noise 噪波修改器
	...
	- Thickness 线宽修改器
	- Time Offset 时间偏移修改器
- 矢量图色彩修改器（Color Modifiers）
	- Hue/Saturation 色相/饱和度修改器
	- Opacity 不透明度编辑
	- Tint 染色修改器

蜡笔视觉特效 (Visual Effects) 是一种图形处理技术，它和 SVG 中的特效扩展一样，将图像处理
算法应用于输入的矢量图形，并得到相应的图形效果输出。Visual Effects 和蜡笔修改器类似，属于 2D
模型的一道加工工序。要观察蜡笔对象的视觉特效，请将 3D 视图着色模式切换为 Rederered 模型，
并为蜡笔对象添加视觉特效设置： 
...
- Rim 边缘视效
- Shadow 阴影视效
- Swirl 漩涡视效，指定一个参考对象，并将靠近此对象的图形，按距离场进行相应强度的旋转。
...
蜡笔的变形是通过蜡笔修改器实现的效果，修改器的属性与动画帧关联，从而实现对曲线顶点的控制。
