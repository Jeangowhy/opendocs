...
调式和调性是两个较为相近的概念，调式是指具体使用的使用音阶调式，比如 C 大调，C 小调。
...
音乐教学中，do re mi fa sol la ti 或者简单上的 1 2 3 4 5 6 7 都是为了简化表达使用的唱名。
为了方便也有使用固定唱名，即 1=C 2=D 3=E 4=F 5=G 6=A 7=B。而在音乐专业或制作者眼中，自然音阶
具体各自的名字：
...
和弦分为 7 个级别，在调式的背景下，依次以各级音为根音，总共可以产生七个级别的和弦，
分别用罗马数字记为 I ii iii IV V vi vii 级和弦。
...
宫、商、角、徵、羽相当于 Do So Re La Mi，没有 Fa Si，可以记为 1 2 3 5 6，或者记住钢琴的黑键就是。
...
然后再计算两音之间所包含的半音数量，即为增、减、大、小，与协和程度直接相关。
...
计算出度数以后，根据前面音程的色彩，大小互换，增减互换，纯不变。
...
五度圈的大调和声的和弦布局，选选择一个调，如 C 大调，那么：
...
掛留和弦 Suspended Chord，三和弦中的大三度音增加大二度，升高一个全音即 sus2 和弦。
或是下降半音，变成 sus4，可用 sus 表示。sus4 和弦由根音，4 度音和 5 度音组成。
